...
*/
	function dynCalendar(objName, callbackFunc,date)
	{
		/**
        * Properties
        */
	    try {
	        ...
	        if (aryDate.length == 3) {
	            this.today = new Date(aryDate[0], aryDate[1] - 1, aryDate[2]);
	        } else {
	            // Todays date
	            this.today = new Date();
	        }
...
*/
	function dynCalendar_show()
	{
		...
		monthnames = new Array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'Novembor', 'December');
		...
		prevImgHTML  = sprintf('<img src="/Images/prev.gif" alt="<<" border="0" />', this.imagesPath);
		prevLinkHTML = sprintf('<a href="javascript: %s.show(%s, %s)">%s</a>',  this.objName, previousMonth, previousYear, prevImgHTML);
		nextImgHTML  = sprintf('<img src="/Images/next.gif" alt=">>" border="0" />', this.imagesPath);
        ...
        */
		...
		html += '<tr>';
		html += '<td class="dynCalendar_Sunday">Sun</td>';
		html += '<td class="dynCalendar_dayname">Mon</td>';
		html += '<td class="dynCalendar_dayname">Tue</td>';
		html += '<td class="dynCalendar_dayname">Wen</td>';
		html += '<td class="dynCalendar_dayname">Thu</td>';
		html += '<td class="dynCalendar_dayname">Fir</td>';
		html += '<td class="dynCalendar_Saturday">Sat</td></tr>';
		html += '<tr>' + ret.join('</tr>\n<tr>') + '</tr>';
		html += '</table>';
...
*/
	function dynCalendar_writeHTML()
	{
		if (is_ie5up || is_nav6up || is_gecko) {
			document.write(sprintf('<a href="javascript: %s.show()"><img src="/Images/dynCalendar.gif" border="0" width="16" height="16" align="absmiddle" /></a>', this.objName, this.imagesPath));
