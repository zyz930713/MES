...
if BOM<>"" then
	where=where&" and JM.BOM_LABEL='"&BOM&"'"
end if
if OBI_Status<>"" then
	where=where&" and J.ERP_JOB_CLOSE_STATUS='"&OBI_Status&"'"
end if

where=where&" and J.STORE_TIME between to_date('"&fromdate&"','yyyy-mm-dd') and to_date('"&todate&"','yyyy-mm-dd')"
if jobnumber<>"" then
	where=where&" and J.JOB_NUMBER like '%"&jobnumber&"%'"
end if

pagepara="&jobnumber="&jobnumber&"&partnumber="&partnumber&"&line="&line&"&code="&code&"&storetype="&storetype&"&checkstatus="&checkstatus&"&fromdate="&fromdate&"&todate="&todate&"&OBI_Status="&OBI_Status&"&BOM="&BOM
pagename="/Job/Store/StoreRecords/StoreRecords.asp"
FactoryRight "J."

SQL="select J.*,JM.ERP_JOB_STATUS,JM.BOM_LABEL from JOB_MASTER_STORE J inner join JOB_MASTER JM on J.JOB_NUMBER=JM.JOB_NUMBER  left join LINE L on J.LINE_NAME=L.LINE_NAME where 1=1 "&factorywhereoutsideand&where&order
'response.Write(SQL)
'response.End()
session("SQL")=SQL
...
<!--#include virtual="/Language/Reports/Store/StoreRecords/Lan_StoreRecords.asp" -->
<!--#include virtual="/Language/Components/Lan_PageSplit.asp" -->
</head>

<body onLoad="language();language_page()">
<form name="form1" method="get" action="/Job/Store/StoreRecords/StoreRecords.asp">
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="19" class="t-c-greenCopy"><span id="inner_Browse"></span></td>
</tr>
<tr>
  <td height="20" colspan="19" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
...
<tr>
  <td height="20" colspan="19"><!--#include virtual="/Components/PageSplit.asp" --></td>
  </tr>
<tr>
...
%>
	<tr>
	  <td height="20"><div align="center"><%=i%></div></td>
		...
		<td><div align="center"><%'if admin=false then%><%=rs("INSPECT_QUANTITY")%><%'else%>
		  <!--<input name="inspect_quanity<%'=i%>" type="text" value="<%=rs("INSPECT_QUANTITY")%>" size="3">
		  <span class="red" style="cursor:hand" onClick="javascript:location.href='InspectUpdate.asp?nid=<%'=rs("NID")%>&new_inspect='+document.all.inspect_quanity<%'=i%>.value+'&path=<%'=path%>&query=<%'=query%>'">¸üÐÂ</span>-->
		  <%'end if%>&nbsp;</div></td>
		<td><div align="center"><%=rs("STORE_QUANTITY")%></div></td>
		<td><div align="center"><%=formatpercent(csng(rs("YIELD")),2,-1)%></div></td>
		<td><div align="center"><% =formatdate(rs("STORE_TIME"),application("longdateformat"))%>
		</div></td>
...
%>
  <tr>
    <td height="20" colspan="19"><div align="center"><span id="inner_NoRecords">No Records</span></div></td>
  </tr>
<%end if
rs.close%>
</table>
