...
FactoryRight "J."
SQL="select J.*,JM.ERP_JOB_STATUS from JOB_MASTER_STORE_PRE J inner join JOB_MASTER JM on J.JOB_NUMBER=JM.JOB_NUMBER inner join FACTORY F on J.FACTORY_ID=F.NID left join LINE L on J.LINE_NAME=L.LINE_NAME where J.JOB_NUMBER is not null "&where&factorywhereoutsideand&order
'response.write sql
session("SQL")=SQL
