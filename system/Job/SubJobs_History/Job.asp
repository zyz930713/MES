...
if seriesgroup<>"" then
SQL="Select INCLUDED_SYSTEM_ITEMS from bar_web.SERIES_GROUP where NID='"&seriesgroup&"'"
...
FactoryRight "P."
SQL="select J.*,P.PART_NUMBER,P.STATIONS_INDEX as P_STATIONS_INDEX,P.STATIONS_TRANSACTION as P_STATIONS_TRANSACTION from bar_hist.JOB J inner join bar_web.PART P on J.PART_NUMBER_ID=P.NID LEFT JOIN STATION S ON J.CURRENT_STATION_ID=S.NID where J.JOB_NUMBER<>'1859097'"&where&factorywhereoutsideand&order

dim StationIndexSQL
StationIndexSQL="select DISTINCT J.STATIONS_INDEX  from bar_hist.JOB J inner join bar_web.PART P on J.PART_NUMBER_ID=P.NID  LEFT JOIN STATION S ON J.CURRENT_STATION_ID=S.NID where J.JOB_NUMBER is not null"&where&factorywhereoutsideand
...
SQLEXCEL=SQLEXCEL & " (select station_start_quantity  From bar_hist.job_stations where job_number=J.JOB_NUMBER and sheet_number=J.SHEET_NUMBER and station_id=j.previous_station_id) as Current_Qty"
SQLEXCEL=SQLEXCEL & " from bar_hist.JOB J inner join bar_web.PART P on J.PART_NUMBER_ID=P.NID where J.JOB_NUMBER is not null"&where&factorywhereoutsideand&order
 
 
rs.open SQL,conn,1,3
 
session("SQL")=SQLEXCEL
session("SQLStation")="select J.*,P.PART_NUMBER,P.STATIONS_INDEX as P_STATIONS_INDEX,P.STATIONS_TRANSACTION as P_STATIONS_TRANSACTION from bar_hist.JOB J inner join bar_web.PART P on J.PART_NUMBER_ID=P.NID where J.JOB_NUMBER is not null"&where&factorywhereoutsideand&" order by J.PART_NUMBER_ID,J.JOB_NUMBER,J.SHEET_NUMBER"
...
<form action="/Job/SubJobs_History/Job.asp" method="get" name="form1" target="_self">
  <table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    ...
    <tr>
      ...
      <td><span id="inner_SearchFamily"></span></td>
      <td><select name="seriesgroup" id="seriesgroup">
        <option value="">Family</option>
        <%FactoryRight "S."%>
		<%=getSeriesGroup("OPTION",seriesgroup,factorywhereoutside,"","")%>
              </select></td>
    ...
    calendar1 = new dynCalendar('calendar1', 'calendar1Callback');
                        </script><select name="fromtime1" id="fromtime1">
...
&nbsp;<select name="fromtime2" id="fromtime2">
    ...
    calendar3 = new dynCalendar('calendar3', 'calendar3Callback');
                        </script><select name="endtime1" id="endtime1">
...
<script language=JavaScript type=text/javascript>
    ...
    calendar4 = new dynCalendar('calendar4', 'calendar4Callback');
    </script><select name="endtime2" id="endtime2">
      ...
      <td>
	  <input name="DisplayStyle" type="radio" value="Station" <%if DisplayStyle="Station" or DisplayStyle="" then%>checked<%end if%>/>Station&nbsp;
	 <input name="DisplayStyle" type="radio" value="Station Group" <%if DisplayStyle="Station Group" then%>checked<%end if%>/>Station Group&nbsp;
	...
	<%
			 ...
			 dim LastStationGroup
			  ...
			  next%>
			   ...
			   <td> <%=left(station_text,len(station_text)-3)%></div></td>
			  </td>
	 <%end if %>
	  <td>
...
%>
    <tr>
      <td height="20" colspan="16"><div align="center"><span id="inner_Records"></span>No Records</div></td>
    </tr>
<%end if
rs.close%>
	<tr>
