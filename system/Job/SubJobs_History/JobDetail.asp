...
SQL="select J.*,P.PART_NUMBER,P.STATIONS_ROUTINE,P.STATIONS_INDEX as P_STATIONS_INDEX,P.STATIONS_TRANSACTION as P_STATIONS_TRANSACTION from bar_hist.JOB J inner join PART P on J.PART_NUMBER_ID=P.NID where J.JOB_NUMBER='"&jobnumber&"' and SHEET_NUMBER='"&sheetnumber&"' and J.JOB_TYPE='"&jobtype&"'"
...
if(request.QueryString("action")="1") then
	SQL="select STATIONS_INDEX,LINE_NAME from bar_hist.JOB where JOB_NUMBER='"&jobnumber&"' and SHEET_NUMBER="&sheetnumber&" and JOB_TYPE='"&jobtype&"'"
	rs.open SQL,conn,1,3
	if not rs.eof then
		stations_index=rs("STATIONS_INDEX")
		line_name=rs("line_name")
...
<body onLoad="language();language_jobnote()">
  <table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    <tr>
      <td height="20" colspan="4" class="t-c-greenCopy"><span id="inner_Browse"></span> (<%=jobnumber%>-<%=repeatstring(sheetnumber,"0",3)%>) </td>
    </tr>
    <tr>
    ...
    <tr>
	  ...
	  <%if admin=true then
	  	...
	  	actionpage=split(apage,",")
        	...
        	<img style="cursor:hand" src="/Images/<%=actionimg(i)%>.gif" alt="click to <%=actionalt(i)%> job" onClick="javascript:window.open('<%=actionpage(i)%>.asp?jobnumber=<%=jobnumber%>&path=<%=path%>&query=<%=query%>','_self')">
        <%	next
		end if%>
&nbsp;</td>
    </tr>
	  ...
	  while not rs.eof
		  ...
		  unit=null		
			if rs("NID")=astation(j) then
				...
				getJobStation_History jobnumber,sheetnumber,jobtype,rs("NID"),arepeated(j),operatorcode,practised,station_start_time,station_close_time'get the finished stations info
...
</div></td><td><div align="center"><%elapsed=datediff("n",station_start_time,station_close_time)%><%=elapsed%>&nbsp;<%=unit%></div></td>
