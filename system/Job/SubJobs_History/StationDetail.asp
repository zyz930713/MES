  ...
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td><table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
        ...
        <tr>
          <%
SQL="Select J.*,A.ACTION_NAME,A.ACTION_CHINESE_NAME from bar_hist.JOB_ACTIONS J right join ACTION A on J.ACTION_ID=A.NID where J.JOB_NUMBER='"&jobnumber&"' and J.SHEET_NUMBER='"&sheetnumber&"' and J.JOB_TYPE='"&jobtype&"'and J.STATION_ID='"&station_id&"' and J.REPEATED_SEQUENCE='"&repeated_sequence&"'"
'response.Write(SQL)
rs.open SQL,conn,1,3
 ...
 <%
  ...
  SQL="Select D.NID,D.DEFECT_NAME,D.DEFECT_CHINESE_NAME,S.STATION_NAME from DEFECTCODE D left join STATION S on D.STATION_ID=S.NID where D.STATION_ID in ('"&station_enter_defectcode&"')"&partwhere
  ...
  k=1
	 while not rs.eof
  		for l=1 to 3
			...
			rsD.close
			%>
          <td width="38%" height="20" class="today" title="<%=rs("NID") %>"><%=k%>.<%= rs("DEFECT_NAME") %>&nbsp;<%= rs("DEFECT_CHINESE_NAME") %>
            <input name="defectcode_id<%=k%>" type="hidden" value="<%=rs("NID")%>"></td>
          <td width="16%" height="20"><input name="defectcode_value<%=k%>" type="text" value="<%= defect_quantity%>" size="6"></td>
          <%rs.movenext
		  	k=k+1
		  	else%>
          <td width="37%" height="20" nowrap class="today">&nbsp;</td>
