...
<%
	...
	if(request.QueryString("Action")="2") then
		SQLStr="SELECT * FROM PIECE_PARTS_SCRAP WHERE SCRAP_TYPE=0"
		...
		if(ERP_Account<>"")then
			SQLStr=SQLStr+" AND SCRAPPING_ACCOUNT='"+ERP_Account+"'"
		end if
		if(ERP_Reason<>"")then
			SQLStr=SQLStr+" AND SCRAPPING_REASON='"+ERP_Reason+"'"
		end if
		if(ERP_Refer<>"")then
			SQLStr=SQLStr+" AND SCRAPPING_REFERENCE='"+ERP_Refer+"'"
		end if
...
<script language="javascript">
	...
	function SearchData()
	{
		form1.action="ManualScrapPiecePartsSearch.asp?Action=2";
		form1.submit();
	}
</script>
</head>
...
<tr>
 ...
 <tr>
  ...
  <tr>
    <td height="20">报废Reference</td>
    <td height="20" colspan="6"><input name="ERP_Refer" type="text" id="ERP_Refer2" onFocus="focushandler(this)" onBlur="blurhandler(this)" size="100" value="<%=Scrap_Ref%>"><br><span id="errorinsertERPrefer" class="red"></span></td>
  </tr>
  <Tr>
  	 <td>报废时间 从:</td> 
...
<table width="100%"  border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
	...
	%>
		<tr>
			...
			<td><%=rs("LINE_NAME")%></td>
			<td><%=rs("COMMENTS")%></td>
			<td><%=getERPAccountName(rs("SCRAPPING_ACCOUNT"))%></td>
			<td><%=getERPReasonName(rs("SCRAPPING_REASON"))%></td>
			<td><%=rs("SCRAPPING_REFERENCE")%></td>
