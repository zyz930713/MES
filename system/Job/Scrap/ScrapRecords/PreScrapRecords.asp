...
if fromdate<>"" then
where=where&" and to_date(J.Scrap_TIME)>=to_date('"&fromdate&"','yyyy-mm-dd')"
end if
if todate<>"" then
where=where&" and to_date(J.Scrap_TIME)<=to_date('"&todate&"','yyyy-mm-dd')"
end if
if OBI_Status<>"" then
...
pagename="/Job/Scrap/ScrapRecords/PreScrapRecords.asp"
FactoryRight "J."
SQL="select J.*,JM.ERP_JOB_STATUS,PF.NID from JOB_MASTER_SCRAP_PRE J inner join JOB_MASTER JM on J.JOB_NUMBER=JM.JOB_NUMBER left join PROFILE_FORM PF on J.PROFILE_FORM_ID=PF.NID inner join FACTORY F on J.FACTORY_ID=F.NID where J.JOB_NUMBER is not null and J.SCRAP_QUANTITY<>0 "&where&factorywhereoutsideand&order
'response.Write(SQL)
session("SQL")=SQL
...
<form name="form1" method="post" action="/Job/Scrap/ScrapRecords/PreScrapRecords.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  ...
  <tr>
    ...
    <td colspan="3"><span id="inner_SearchFrom"></span>
  ...
  <td height="20" colspan="14" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%" class="t-c-greenCopy"><span id="inner_User"></span>: 
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('ScrapRecords_Export.asp')"><img src="/Images/EXCEL.gif" width="22" height="22"></span></div></td>
      </tr>
    </table></td>
</tr>
<tr>
...
<tr>
  ...
  <td class="t-t-Borrow"><div align="center"><img src="/Images/Accend.gif" alt="Click to accend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=J.Scrap_TIME&ordertype=asc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'"><span id="inner_ScrapTime"></span><img src="/Images/Decend.gif" alt="Click to decend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=J.Scrap_TIME&ordertype=desc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'"></div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_Reason"></span></div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_FormID"></span></div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_Note"></span></div></td>
  <td class="t-t-Borrow" align="center"><span id="td_ScrapAccount"></span></td>
  <td class="t-t-Borrow" align="center"><span id="td_ScrapReason"></span></td>
  <td class="t-t-Borrow" align="center"><span id="td_ScrapReference"></span></td>
  <td class="t-t-Borrow"><div align="center"><span id="td_ERPJobStatus"></span></div></td>
  </tr>
<%
i=1
if not rs.eof then
...
%>
	<tr>
	  <td height="20"><div align="center"><%=i%></div></td>
		...
		<td><div align="center"><%=rs("SCRAP_CODE")%></div></td>
		...
		<td><%=rs("NOTE")%>&nbsp;</td>
		<td><%=rs("scrap_account")%></td>
		<td><%=rs("SCRAP_REASON_ID")%></td>
	  <td><%=rs("SCRAP_REFERENCE")%>&nbsp;</td>
...
%>
  <tr>
    <td height="20" colspan="14"><div align="center"><span id="inner_NoRecords"></span></div></td>
  </tr>
<%end if
rs.close%>
</table>
