...
if sheetnumber="" then
SQL="Select JD.*,S.STATION_NAME,D.DEFECT_NAME,D.DEFECT_CHINESE_NAME from JOB_DEFECTCODES JD inner join STATION S on JD.STATION_ID=S.NID left join DEFECTCODE D on JD.DEFECT_CODE_ID=D.NID where JD.JOB_NUMBER='"&jobnumber&"'"
else
SQL="Select JD.*,S.STATION_NAME,D.DEFECT_NAME,D.DEFECT_CHINESE_NAME from JOB_DEFECTCODES JD inner join STATION S on JD.STATION_ID=S.NID left join DEFECTCODE D on JD.DEFECT_CODE_ID=D.NID where JD.JOB_NUMBER='"&jobnumber&"' and JD.SHEET_NUMBER='"&sheetnumber&"'"
end if
...
<tr>
  ...
  <td height="20"><div align="center"><%= rs("DEFECT_NAME") %> (<%= rs("DEFECT_CHINESE_NAME") %>)</div></td>
