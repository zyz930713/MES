...
for i=1 to request.Form("idcount")
	if request.Form("id"&i)="1" then
		SQL="select JOB_NUMBER,STATION_ID,OPERATOR_CODE,STATUS,START_TIME,CLOSE_TIME from JOB_STATIONS where JOB_NUMBER='"&jobnumber&"' and SHEET_NUMBER='"&sheetnumber&"' and JOB_TYPE='"&jobtype&"' and STATION_ID='"&request.Form("station_id"&i)&"' and REPEATED_SEQUENCE='"&request.Form("repeated_sequence"&i)&"'"
