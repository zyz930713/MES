...
FactoryRight "J."
SQL="select J.NID,J.TRANSACTION_ID,J.JOB_NUMBER,J.PART_NUMBER_TAG,J.LINE_NAME,J.OLD_STORE_QUANTITY,J.NEW_STORE_QUANTITY,J.CHANGE_CODE ,J.CHANGE_TIME ,NVL(J.ALLOW_ERP_RESUBMIT,'Y') AS ALLOW_ERP_RESUBMIT,J.ERP_ERROR_CODE,J.ERP_LAST_UPDATE_TIME,J.ERP_ERROR_EXPLANATION,JM.ERP_JOB_STATUS from JOB_MASTER_STORE_CHANGE J inner join JOB_MASTER JM on J.JOB_NUMBER=JM.JOB_NUMBER left join FACTORY F on J.FACTORY_ID=F.NID where TRANSACTION_ID IS NOT NULL AND ERP_JOB_CLOSE_STATUS IN ('VALIDATION_ERROR', 'FAILURE') AND NVL(JM.OBI_ENABLED, 'Y') = 'Y'"&where&factorywhereoutsideand&order
'response.Write(SQL)
session("SQL")=SQL
...
<!--#include virtual="/Language/OBI/Lan_JobClosingReturn.asp" -->
</head>

<body onLoad="language();language_page()">
<form name="form1" method="get" action="/OBI/JobStoreReturn.asp">
...
<tr>
  ...
  <td class="t-t-Borrow"><div align="center"><span id="inner_OldStoreQuantity"></span></div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_NewStoreQuantity"></span></div></td>
...
<form action="/OBI/JobStoreReturn1.asp" method="post" name="checkform" id="checkform">
<%
i=1
if not rs.eof then
...
<%if OBI=true then%>
		<td><div align="center"><%if rs("ALLOW_ERP_RESUBMIT")="Y" then%><img src="/Images/Enabled.gif" width="10" height="10" style="cursor:hand" onClick="javascript:location.href='DisableJobStoreReturnResubmit.asp?p_nid=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>'"><%else%><img src="/Images/Disabled.gif" width="10" height="10" style="cursor:hand" onClick="javascript:location.href='EnableJobStoreReturnResubmit.asp?p_nid=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>'"><%end if%>&nbsp;</div></td>		<%end if%>
		<td>
		  <div align="center">
