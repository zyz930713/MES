...
FactoryRight "J."
'SQL="select J.NID,J.TRANSACTION_ID,J.JOB_NUMBER,J.PART_NUMBER_TAG,J.LINE_NAME,J.SCRAP_QUANTITY,J.SCRAP_ACCOUNT,J.SCRAP_REASON_ID,J.SCRAP_REFERENCE,NVL(J.ALLOW_ERP_RESUBMIT,'Y') AS ALLOW_ERP_RESUBMIT,J.ERP_LAST_UPDATE_TIME,J.ERP_ERROR_CODE,J.ERP_ERROR_EXPLANATION,JM.ERP_JOB_STATUS from JOB_MASTER_SCRAP J inner join JOB_MASTER JM on J.JOB_NUMBER=JM.JOB_NUMBER left join FACTORY F on J.FACTORY_ID=F.NID where TRANSACTION_ID IS NOT NULL AND ERP_JOB_CLOSE_STATUS IN ('VALIDATION_ERROR', 'FAILURE') AND NVL(JM.OBI_ENABLED, 'Y') = 'Y'"&where&factorywhereoutsideand&order
SQL="select J.NID,J.TRANSACTION_ID,J.JOB_NUMBER,J.PART_NUMBER_TAG,J.LINE_NAME,J.SCRAP_QUANTITY,J.SCRAP_ACCOUNT,J.SCRAP_REASON_ID,NVL(J.ALLOW_ERP_RESUBMIT,'Y') AS ALLOW_ERP_RESUBMIT,J.ERP_LAST_UPDATE_TIME,J.ERP_ERROR_CODE,J.ERP_ERROR_EXPLANATION,JM.ERP_JOB_STATUS from JOB_MASTER_SCRAP J inner join JOB_MASTER JM on J.JOB_NUMBER=JM.JOB_NUMBER left join FACTORY F on J.FACTORY_ID=F.NID where TRANSACTION_ID IS NOT NULL AND ERP_JOB_CLOSE_STATUS IN ('VALIDATION_ERROR', 'FAILURE') AND NVL(JM.OBI_ENABLED, 'Y') = 'Y'"&where&factorywhereoutsideand&order
...
<!--#include virtual="/Language/OBI/Lan_JobClosingScrap.asp" -->
</head>

<body onLoad="language();language_page()">
<form name="form1" method="get" action="/OBI/JobClosingScrap.asp">
...
<script language=JavaScript type=text/javascript>
    ...
    <td><select name="errorcode" id="errorcode">
	  ...
	  <option value="XX_MISSING_SCRAP_REASON" <%if errorcode="XX_MISSING_SCRAP_REASON" then%>selected<%end if%>>XX_MISSING_SCRAP_REASON</option>
	  <option value="XX_MISSING_SCRAP_ACCOUNT" <%if errorcode="XX_MISSING_SCRAP_ACCOUNT" then%>selected<%end if%>>XX_MISSING_SCRAP_ACCOUNT</option>
  ...
  <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%" class="t-c-greenCopy"><span id="inner_User"></span>: 
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('JobClosingScrap_Export.asp')"><img src="/Images/EXCEL_Middle.gif" width="22" height="22"></span>&nbsp;</div></td>
      </tr>
    </table>	</td>
</tr>
<tr>
...
<tr>
  ...
  <td class="t-t-Borrow"><div align="center"><span id="inner_ScrapAccount"></span></div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_ScrapReason"></span></div></td>
  <td class="t-t-Borrow"><div align="center">Reference</div></td>
  <td class="t-t-Borrow"><div align="center">ERP Job Status</div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_ErrorCode"></span></div></td>
  <td class="t-t-Borrow"><div align="center"><span id="inner_Error"></span></div></td>
  </tr>
<form action="/OBI/JobClosingScrap1.asp" method="post" name="checkform" id="checkform">
<%
i=1
if not rs.eof then
...
<%if OBI=true then%>
		<td><div align="center"><%if rs("ALLOW_ERP_RESUBMIT")="Y" then%><img src="/Images/Enabled.gif" width="10" height="10" style="cursor:hand" onClick="javascript:location.href='DisableJobClosingScrapResubmit.asp?p_nid=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>'"><%else%><img src="/Images/Disabled.gif" width="10" height="10" style="cursor:hand" onClick="javascript:location.href='EnableJobClosingScrapResubmit.asp?p_nid=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>'"><%end if%>&nbsp;</div></td>		<%end if%>
		<td>
		  <div align="center">
	  ...
	  <td><div align="center">
		...
		<td><div align="center"><%' =rs("SCRAP_REFERENCE")%>&nbsp;</div></td>
		<td><div align="center">
		  <% =rs("ERP_JOB_STATUS")%>&nbsp;
	    </div></td>
		<td><div align="center">
