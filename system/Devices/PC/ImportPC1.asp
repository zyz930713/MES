...
SQL="delete from BARCODE_DESKTOP"
rs.open SQL,conn,3,3
SQL="select * from BARCODE_DESKTOP"
rs.open SQL,conn,3,3
...
Do
rs.AddNew
rs("NID")="DE"&NID_SEQ("DESKTOP")
rs("COMPUTER_NAME") = trim(Sheet.Cells(i,1).Text)
rs("SERVICE_TAG") = trim(Sheet.Cells(i,2).Text)
rs("MAC_ADDRESS") = trim(Sheet.Cells(i,3).Text)
rs("MODEL") = trim(Sheet.Cells(i,4).Text)
rs.Update
i = i + 1
...
%>
<script language="JavaScript">
alert("Successfully upload desktops")
window.close()
</script>
<%
end if
