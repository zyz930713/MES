...
pagename="/Reports/Process/PartWIP/PartWIPList.asp"
SQL="select * from PART_WIP_LIST where NID is not null "&where&order
session("SQL")=SQL
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="5" class="t-c-greenCopy"><span>Search Part WIP Reports </span></td>
  </tr>
  <tr>
...
<tr>
  <td height="20" colspan="5" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="48%">Browse Part WIP Report </td>
		...
		<%if admin=true then%>
		<form action="/Reports/Process/PartWIP/GeneratePartWIP.asp" method="post" name="form2" target="_self" onSubmit="return form2check()">
<tr>
  <td height="20" colspan="5" class="t-c-greenCopy">
		  Section type: <%FactoryRight "S."%><%=getSection("RADIO","",factorywhereoutside,"","")%>
          <br>
          <input name="Generate" type="submit" id="Generate" value="Generate Part WIP report at once">
</td>
</tr>
  </form>
		  <%end if%>
...
<tr>
  <td height="20"><div align="center"><% =(session("strpagenum")-1)*pagesize_s+i%></div></td>
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='DeletePartWIPReport.asp?id=<%=rs("NID")%>&part_WIP_name=<%=rs("PART_WIP_NAME")%>&path=<%=path%>&query=<%=query%>'">Delete</span></div></td>
    <td height="20"><div align="center"><% =formatdate(rs("REPORT_TIME"),application("longdateformat"))%></div></td>
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('PartWIPReport.asp?part_WIP_id=<%=rs("NID")%>&part_WIP_name=<%=rs("PART_WIP_NAME")%>&part_WIP_report_time=<%=rs("REPORT_TIME")%>&section_id=<%=rs("SECTION_ID")%>')"><%= rs("PART_WIP_NAME") %></span>&nbsp;</div></td>
