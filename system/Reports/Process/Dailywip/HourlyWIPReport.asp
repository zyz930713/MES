...
SQL="Select S.STATION_NAME,L.LINE_NAME,nvl(avg(HWD.QUANTITY),0) as QUANTITY,HWL.REPORT_TIME from BAR_REPORT.HOURLY_WIP_DETAIL HWD inner join BAR_REPORT.HOURLY_WIP_LIST HWL on HWD.WIP_ID=HWL.NID inner join BAR_WEB.LINE L on HWD.LINE_ID=L.LINE_NAME inner join BAR_WEB.STATION S on HWD.STATION_ID=S.NID where and to_char(HWL.REPORT_TIME,'yyyy-mm-dd')>='"&formatdate(from_day,application("F_shortdateformat"))&"' and to_char(HWL.REPORT_TIME,'yyyy-mm-dd')<='"&formatdate(to_day,application("F_shortdateformat"))&"' and S.WIP_REPORT_COLUMN=1 and S.FACTORY_ID='"&factory_id&"'"&where&order
else
SQL="Select S.STATION_NAME,L.LINE_NAME,HWD.QUANTITY,HWL.REPORT_TIME from BAR_REPORT.HOURLY_WIP_DETAIL HWD inner join BAR_REPORT.HOURLY_WIP_LIST HWL on HWD.WIP_ID=HWL.NID inner join BAR_WEB.LINE L on HWD.LINE_ID=L.LINE_NAME inner join BAR_WEB.STATION S on HWD.STATION_ID=S.NID where HWD.WIP_ID='"&WIP_ID&"' and S.WIP_REPORT_COLUMN=1 and S.FACTORY_ID='"&factory_id&"'"&where&order
end if
session("SQL")=SQL
'response.Write(SQL)
...
<body>
<form action="/Reports/Process/DailyWIP/HourlyWIPReport.asp" method="get" name="form1" target="_self">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="5" class="t-c-greenCopy">Browse Recorded WIP -- <%=WIP_NAME%> (<% =formatdate(from_day,application("longdateformat"))%> to <% =formatdate(to_day,application("longdateformat"))%>)</td>
</tr>
<tr>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td height="20"><select name="line" id="line">
        ...
        <%=getLine("OPTION_LINE_NAME",line,factorywhereoutside," order by L.LINE_NAME","")%>
      </select></td>
