...
SQL="select L.NID,L.LINE_NAME from LINE L where L.STATUS=1 and L.FACTORY_ID='"&factory_id&"' order by L.LINE_NAME asc"
...
rsT.open SQLT,conn,1,3
	...
	While not rs.eof	
		...
		While not rsT.eof	
			...
			sheet.cells(i,j).value=getRecordedWIP_WTD(rs("LINE_NAME"),rsT("NID"),job_numbers,from_day,to_day)
			else
			sheet.cells(i,j).value=getRecordedWIP(WIP_ID,rs("LINE_NAME"),rsT("NID"))
			end if
		j=j+1
		rsT.movenext
		wend
