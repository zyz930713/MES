...
<!--#include virtual="/Functions/GetOutputJobQuantity.asp" -->
<%
...
job_numbers=request.QueryString("job_numbers")
output_name=trim(request.QueryString("output_name"))
output_report_time=trim(request.QueryString("output_report_time"))
...
pagename="/Reports/Output/OutputPartDetail.asp"
...
SQL="select J.JOB_NUMBER,J.SHEET_NUMBER from JOB J where "&jobwhere&order
session("SQL")=SQL
...
<tr>
  <td height="20" colspan="<%=Tcount%>" class="t-c-greenCopy">Browse Recorded Output -- <%=line_name%> of <%=output_name%> from <%=fromtime%> to <%=totime%> (<% =formatdate(output_report_time,application("longdateformat"))%>)</td>
</tr>
<tr>
  <td height="20" colspan="<%=Tcount%>" class="t-c-greenCopy">User:
    <% =session("User") %></td></tr>
<tr>
...
<tr>
  <td height="20"><div align="center"><% =i%></div></td>
    <td><div align="center"><a href="/Job/SubJobs/JobDetail.asp?jobnumber=<%=rs("JOB_NUMBER")%>&sheetnumber=<%=rs("SHEET_NUMBER")%>" target="_blank"><%= rs("JOB_NUMBER") %>-<%=repeatstring(rs("SHEET_NUMBER"),"0",3)%></a></div></td>
    <%
	  if not rsT.eof then
	  j=0
	  While not rsT.eof%>
	  <div align="center"><td><div align="center"><%station_quantity=getOutputJobQuantity(fromtime,totime,rs("JOB_NUMBER"),rs("SHEET_NUMBER"),rsT("NID"))%><%=station_quantity%>&nbsp;</div></td></div>
	  <%
