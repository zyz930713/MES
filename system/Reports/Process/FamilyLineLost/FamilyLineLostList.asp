...
order=" order by FL.CHART_WEEK desc"
else
order=" order by "&ordername&" "&ordertype
end if
where=""
if reportname<>"" then
where=where&" and FL.FAMILY_LINELOST_NAME='"&reportname&"'"
end if
if fromdate<>"" then
...
pagename="/Reports/Process/FamilyLineLost/FinalFamilyYieldList.asp"
SQL="select FL.*,F.FACTORY_NAME from FAMILY_LINELOST_LIST FL inner join FACTORY F on FL.FACTORY_ID=F.NID where FL.NID is not null "&where&order
session("SQL")=SQL
...
<form name="form1" method="post" action="/Reports/Process/FamilyLineLost/FamilyLineLostList.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="5" class="t-c-greenCopy"><span>Search Family Line Lost Reports </span></td>
  </tr>
  <tr>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#006600" bordercolordark="#FFFFFF">
  <%if admin=true then%>
  <form action="/Reports/Process/FamilyLineLost/GenerateFamilyLineLost.asp" method="post" name="form2" target="_self" onSubmit="return form2check()">
  <tr>
...
&nbsp; <input name="jtohour" type="text" id="jtohour" value="00:00" size="5"></td>
  ...
  <tr>
    <td height="20" colspan="4"><input name="Generate" type="submit" class="t-b-Yellow" id="Generate" value="Generate Family Line Lost Report at once"></td>
    </tr>
  </form>
  <%end if%>
</table>
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="9" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="48%">Browse Family Line Lost Report </td>
...
%>
  <tr>
    ...
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='DeleteFamilyLineLostReport.asp?id=<%=rs("NID")%>&familylinelost_name=<%=rs("FAMILY_LINELOST_NAME")%>&path=<%=path%>&query=<%=query%>'">Delete</span></div></td>
    <td height="20">
      <div align="center">
        <% =formatdate(rs("REPORT_TIME"),application("longdateformat"))%>
      </div></td>
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('FamilyLineLostReport.asp?familylinelost_id=<%=rs("NID")%>&familylinelost_name=<%=rs("FAMILY_LINELOST_NAME")%>&familylinelost_report_time=<%=rs("REPORT_TIME")%>&from_time=<%=rs("FROM_TIME")%>&to_time=<%=rs("TO_TIME")%>&factory_id=<%=rs("FACTORY_ID")%>')"><%= rs("FAMILY_LINELOST_NAME") %></span>&nbsp;</div></td>
    <td><div align="center"><%if rs("CHART_WEEK")<>"" then%><span class="red" style="cursor:hand" onClick="javascript:window.open('http://<%=application("ChartServer")%>/KES1_Barcode/FinalFamilyYieldChart.asp?yearnumber=<%=rs("CHART_YEAR")%>&fromww=1&toww=<%=datepart("ww",rs("TO_TIME"))%>&factory_id=<%=rs("FACTORY_ID")%>')"><img src="/Images/IconReportChart.gif" width="22" height="22"></span><%else%>&nbsp;<%end if%></div></td>
