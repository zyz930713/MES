...
SQL="select * from OUTPUT_LIST where NID is not null "&where&order
session("SQL")=SQL
...
<body>
<form name="form1" method="post" action="/Reports/Process/FamilyOutput/OutputList.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="5" class="t-c-greenCopy"><span>Search Output Reports </span></td>
  </tr>
  <tr>
...
<tr>
  <td height="20" colspan="7" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="47%">Browse Output Report </td>
		...
		<%if admin=true then%>
          <form action="/Reports/Process/FamilyOutput/GenerateOutput.asp" method="post" name="form2" target="_self" onSubmit="return form2check()">
<tr>
  <td height="20" colspan="7" class="t-c-greenCopy">
		  Section type: 
		    <%FactoryRight "S."%><%=getSection("RADIO","",factorywhereoutside,"","")%><br>
		  <input name="Generate" type="submit" id="Generate" value="Generate Output report at once">
...
<tr>
  <td height="20"><div align="center"><% =(session("strpagenum")-1)*pagesize_s+i%></div></td>
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='DeleteOutputReport.asp?id=<%=rs("NID")%>&output_name=<%=rs("OUTPUT_NAME")%>&path=<%=path%>&query=<%=query%>'">Delete</span></div></td>
    <td height="20"><div align="center"><% =formatdate(rs("REPORT_TIME"),application("longdateformat"))%></div></td>
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('OutputReport.asp?output_id=<%=rs("NID")%>&output_name=<%=rs("OUTPUT_NAME")%>&output_report_time=<%=rs("REPORT_TIME")%>&fromtime=<%=rs("FROM_TIME")%>&totime=<%=rs("TO_TIME")%>&section_id=<%=rs("SECTION_ID")%>')"><%= rs("OUTPUT_NAME") %></span>&nbsp;</div></td>
