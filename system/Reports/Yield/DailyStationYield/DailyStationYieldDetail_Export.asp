...
total_output_quantity=0
	...
	if report_type="" then
	sheet.cells(1,1).value="Daily Station Yield Detail on "&report_day
	else
	sheet.cells(1,1).value="Weekly Station Yield Detail from "&from_time&" to "&to_time
	end if
	...
	sheet.cells(2,6).value="Defectcode Quantity"
...
while not rs.eof
	...
	sheet.cells(i,5).value=rs("STATION_START_QUANTITY")
	sheet.cells(i,6).value=rs("GOOD_QUANTITY")
	sheet.cells(i,7).value=formatpercent(csng(rs("GOOD_QUANTITY"))/csng(rs("STATION_START_QUANTITY")),2,-1)
	total_input_quantity=total_input_quantity+cint(rs("STATION_START_QUANTITY"))
	total_output_quantity=total_output_quantity+cint(rs("GOOD_QUANTITY"))
i=i+1
rs.movenext
wend
	...
	sheet.cells(i,7).value=formatpercent(total_yield,2,-1)
