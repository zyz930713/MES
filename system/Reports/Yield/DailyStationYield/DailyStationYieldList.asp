...
<!--#include virtual="/Functions/GetDailyStationYieldByDay.asp" -->
...
<body>
<form action="/Reports/Yield/DailyStationYield/DailyStationYieldList.asp" method="post" name="form1" target="_self" onSubmit="return formcheck()">
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#006600" bordercolordark="#DFDFDF">
    ...
    <tr>
      <td class="t-b-blue"><span>Task Type </span></td>
      <td><select name="profile_task_id" id="profile_task_id">
        <option value="">--Select--</option>
        <%= getProfileTask("OPTION",profile_task_id," and (lower(TASK_NAME) like '%daily station yield%')"," order by TASK_NAME","") %>
      </select></td>
...
&nbsp;</td>
      ...
      <input name="weekly" type="button" class="t-t-DarkBlue" id="weekly" value="Weekly Report" onClick="location.href='WeeklyStationYieldList.asp'"></td>
    </tr>
  </table>
</form>
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000000" bordercolordark="#FFFFFF" byield="1" byieldcolorlight="#666666" byieldcolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="<%=datediff("d",cdate(start_day),cdate(close_day))+3%>" class="t-c-greenCopy">
      <table width="100%" cellpadding="0" cellspacing="0" byield="0">
        <tr>
          <td>Daily Station Yield in <%=datediff("d",cdate(start_day),cdate(close_day))+1%> days</td>
        </tr>
      </table>    </td>
  </tr>
  <tr class="t-b-blue">
    ...
    <td width="344" height="20" class="<%if weekday(this_day)=1 then%>red<%elseif weekday(this_day)=7 then%>green<%end if%>"><div align="center"><%=this_day%>&nbsp;(<%=shortweekdayconvert(weekday(this_day))%>)&nbsp;<span style="cursor:hand" class="red" onClick="window.open('RegenerateDailyStationYieldReport.asp?profile_task_id=<%=profile_task_id%>&factory_id=<%=factory_id%>&from_time=<%=this_day%>&to_time=<%=dateadd("d",1,this_day)%>')" title="Regenerate">R</span></div></td>
  ...
  <tr>
	...
	this_day=start_day
	while this_day<=close_day%>
    <td height="20" <%if this_day=date() then%>class="t-t-Borrow"<%end if%>><span style="cursor:hand" onClick="window.open('DailyStationYieldDetail.asp?station_id=<%=rs1("NID")%>&report_day=<%=this_day%>')"><%=formatpercent(GetDailyStationYieldByDay(rs1("NID"),this_day),2,-1)%></span></td>
	<%this_day=dateadd("d","1",this_day)
	wend%>
  </tr>
  <%rs1.movenext
