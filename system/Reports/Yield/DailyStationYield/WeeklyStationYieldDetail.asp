...
order=" order by JS.JOB_NUMBER,JS.SHEET_NUMBER"
else
order=" order by "&ordername&" "&ordertype
end if
SQL="select J.JOB_NUMBER,J.SHEET_NUMBER,J.JOB_TYPE,J.PART_NUMBER_TAG,J.LINE_NAME,S.STATION_NAME,JS.STATION_START_QUANTITY,JS.GOOD_QUANTITY FROM JOB_STATIONS JS inner join JOB J on JS.JOB_NUMBER=J.JOB_NUMBER and JS.SHEET_NUMBER=J.SHEET_NUMBER inner join STATION S on JS.STATION_ID=S.NID WHERE JS.CLOSE_TIME>=to_date('"&from_time&"','yyyy-mm-dd hh24:mi:ss') and JS.CLOSE_TIME<=to_date('"&to_time&"','yyyy-mm-dd hh24:mi:ss') and JS.STATION_ID='"&station_id&"'"&order
session("SQL")=SQL
'response.Write(SQL)
...
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy">Browse Recorded Weekly Station Yield -- from <%=from_time%> to <%=to_time%></td>
</tr>
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%">User:
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyStationYieldDetail_Export.asp?report_type=WE&station_id=<%=station_id%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td></tr>
<!--<tr>
...
<tr>
  <td height="20"><div align="center"><% =i%></div></td>
    <td><div align="center"><span style="cursor:hand" class="d_link" onClick="javascript:window.open('/Job/SubJobs/JobDetail.asp?jobnumber=<%=rs("JOB_NUMBER")%>&sheetnumber=<%=rs("SHEET_NUMBER")%>&jobtype=<%=rs("JOB_TYPE")%>','_blank')"><%=rs("JOB_NUMBER")%>-<%=replace(rs("JOB_TYPE"),"N","")&repeatstring(rs("SHEET_NUMBER"),"0",3)%></span></div></td>
	 ...
	 <td><div align="center"><%= rs("STATION_START_QUANTITY") %></div></td>
	 <td><div align="center"><%= rs("GOOD_QUANTITY") %></div></td>
	 <td><div align="center"><%= formatpercent(csng(rs("GOOD_QUANTITY"))/csng(rs("STATION_START_QUANTITY")),2,-1)%></div></td>
    </tr>
<%
total_input=total_input+csng(rs("STATION_START_QUANTITY"))
total_output=total_output+csng(rs("GOOD_QUANTITY"))
i=i+1
rs.movenext
wend
