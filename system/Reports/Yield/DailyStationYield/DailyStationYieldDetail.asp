...
SQL="select J.JOB_NUMBER,J.SHEET_NUMBER,J.JOB_TYPE,J.PART_NUMBER_TAG,J.LINE_NAME,S.STATION_NAME,JS.STATION_START_QUANTITY,JS.GOOD_QUANTITY FROM JOB_STATIONS JS inner join JOB J on JS.JOB_NUMBER=J.JOB_NUMBER and JS.SHEET_NUMBER=J.SHEET_NUMBER inner join STATION S on JS.STATION_ID=S.NID WHERE to_char(JS.CLOSE_TIME,'yyyy-mm-dd')='"&formatdate(report_day,application("F_shortdateformat"))&"' and JS.STATION_ID='"&station_id&"'"&order
session("SQL")=SQL
'response.Write(SQL)
...
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy">Browse Recorded Daily Station Yield -- on <%=report_day%></td>
</tr>
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%">User:
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyStationYieldDetail_Export.asp?station_id=<%=station_id%>&report_day=<%=report_day%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td></tr>
<!--<tr>
