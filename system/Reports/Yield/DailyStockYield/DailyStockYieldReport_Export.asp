...
rs.close
	...
	sheet.cells(1,1).value="Daily Stock Yield Report from "&formatdate(from_time,application("longdateformat"))&" to "&formatdate(to_time,application("longdateformat"))
	...
	sheet.cells(3,i+3).value="Internal"
...
for i=0 to ubound(a_family_names)
	...
	SQL1="select * from DAILY_FAMILYSTOCK_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and FACTORY_ID='"&factory_id&"' and FAMILY_NAME='"&a_family_names(i)&"' and LINE_NAME='"&a_line_names(j)&"' and to_char(LINE_DAY,'yyyy-mm-dd')='"&formatdate(current_date,application("F_shortdateformat"))&"'"
		...
		rs1.close
		sheet.cells(m,k).value=stock_input_quantity
		sheet.cells(m,k+1).value=stock_output_quantity
		sheet.cells(m,k+2).value=formatpercent(stock_yield,2,-1)
		sheet.cells(m,k+2).Interior.ColorIndex=getNormalYieldExcelColor(stock_yield,family_target_yield)
		sheet.cells(m,k+3).value=formatpercent(family_internal_yield,2,-1)
		...
		sheet.cells(m,k+3).value=formatpercent(line_internal_yield,2,-1)
...
sheet.cells(m,i+2).Interior.ColorIndex=getNormalYieldExcelColor(total_yield,factory_target)
...
sheet.cells(m,i+2).Interior.ColorIndex=getNormalYieldExcelColor(overall_yield,factory_target)
sheet.cells(m,i+3).Interior.ColorIndex=xlColorRose
sheet.cells(m,i+4).value=formatpercent(factory_target,2,-1)
sheet.cells(m,i+4).Interior.ColorIndex=xlColorRose
