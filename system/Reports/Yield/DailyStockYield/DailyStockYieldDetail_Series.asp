...
rs.close
SQL="select * from JOB_MASTER_STORE where FACTORY_ID='"&factory_id&"' and EXACT_INSTR('"&included_system_items&"',PART_NUMBER_TAG,',')>0 and LINE_NAME='"&line_name&"' and STORE_TIME>=to_date('"&from_time&"','yyyy-mm-dd hh24:mi:ss') and STORE_TIME<=to_date('"&to_time&"','yyyy-mm-dd hh24:mi:ss') "&order
'response.Write(SQL)
session("SQL")=SQL
rs.open SQL,conn,1,3
%>
