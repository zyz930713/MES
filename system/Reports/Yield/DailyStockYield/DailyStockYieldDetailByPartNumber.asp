...
pagename="/Reports/Yield/DailyLineYield/DailyLineDetailbyPartNumber.asp.asp"
...
<tr>
  <td height="20" colspan="7">Sort by: 
    <input name="ByJob" type="radio" value="1" onClick="javascript:window.open('DailyLineYieldDetailByJobNumber.asp?line_name=<%=line_name%>&from_time=<%=from_time%>&to_time=<%=to_time%>','_self')">
    Job Number
    <input name="ByNormal" type="radio" value="1" onClick="javascript:window.open('DailyLineYieldDetail.asp?line_name=<%=line_name%>&from_time=<%=from_time%>&to_time=<%=to_time%>','_self')">
    Normal </td>
</tr>
  <tr>
...
%>
    <tr>
      ...
      <td height="20"><div align="center"><span style="cursor:hand" class="d_link" onClick="javascript:window.open('/Job/SubJobs/JobDetail.asp?jobnumber=<%=this_jobnumber(u)%>&sheetnumber=<%=this_sheetnumber(u)%>&jobtype=<%=this_jobtype(u)%>','_blank')"><%=this_jobnumber(u)%>-<%=replace(this_jobtype(u),"N","")&repeatstring(this_sheetnumber(u),"0",3)%></span>&nbsp;
          <%if DBA=true then%>
