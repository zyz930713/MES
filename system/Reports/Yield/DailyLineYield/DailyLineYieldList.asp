...
<!--#include virtual="/Functions/GetDailyLineYieldByDay.asp" -->
...
<script language="JavaScript" src="/Reports/Yield/DailyLineYield/formcheck.js" type="text/javascript"></script>
</head>

<body>
<form action="/Reports/Yield/DailyLineYield/DailyLineYieldReport.asp" method="post" name="form1" target="_blank" onSubmit="return formcheck()">
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#006600" bordercolordark="#DFDFDF">
    ...
    <tr>
      ...
      <td><select name="profile_task_id" id="profile_task_id">
        ...
        <%= getProfileTask("OPTION",""," and (lower(TASK_NAME) like '%daily family line yield%' or lower(TASK_NAME) like '%daily series line yield%')"," order by TASK_NAME","") %>
      </select></td>
...
<input name="tohour" type="text" id="tohour" value="14:30" size="5"></td>
      <td><input name="Generate" type="submit" class="t-b-Yellow" id="Generate" value="Multi-Day Report">
        &nbsp;
      <input name="ontime" type="button" class="t-t-DarkBlue" id="ontime" value="Ontime Report" onClick="location.href='OnTimeLineYield.asp'"></td>
    </tr>
  </table>
</form>
    ...
    <td height="20" colspan="7" class="t-c-greenCopy">
      <table width="100%" cellpadding="0" cellspacing="0" byield="0">
        <tr>
          ...
          <td><div align="center">Daily Line Yield of <%=monthconvert(thisyieldmonth)%></div></td>
  ...
  <tr>
    <%		bgimg=""
			...
			if weekday(dates)=i and year(dates)=thisyieldyear and month(dates)=thisyieldmonth then
				yields=GetDailyLineYieldByDay(dates)
				if i=1 then
