...
pagename="/Reports/Yield/DailyLineYield/DailyLineYieldReport_Series.asp"
dateinterval=datediff("d",from_time,to_time)+1
cells=(dateinterval+1)*4+3
series_names=getSeries("TEXT",""," where S.FACTORY_ID='"&factory_id&"'"," order by S.SERIES_NAME","|")
if series_names<>"" then
series_names=left(series_names,len(series_names)-1)
a_series_names=split(series_names,"|")
else
dim a_series_names(0)
end if
...
<tr>
  <td height="20" colspan="<%=cells%>" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="50%"><div align="right"><span style="cursor:hand" title="Regenerate Report" onClick="javascript:window.open('RegenerateDailyLineYieldReport.asp?profile_task_id=<%=profile_task_id%>&factory_id=<%=factory_id%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/IconRegenerate.gif" alt="Regenerate Report" width="22" height="22" align="absmiddle"></span>&nbsp;<span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyLineYieldReport_Export.asp?factory_id=<%=factory_id%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/EXCEL_Middle.gif" width="22" height="22" align="absmiddle"></span></div></td>
      </tr>
    </table></td>
</tr>
<tr>
...
<tr>
  ...
  <td rowspan="2" class="t-t-Borrow"><div align="center">Series Name </div></td>
  ...
  <td height="20" colspan="4" class="<%if weekday(current_date)=1 then%>t-b-Red<%elseif weekday(current_date)=7 then%>t-c-greenCopy<%else%>t-t-Borrow<%end if%>"><div align="center"><%=current_date%> (<%=shortweekdayconvert(weekday(current_date))%>)</div></td>
  <%current_date=dateadd("d",1,current_date)
	loop%>
	<td height="20" colspan="4" class="t-b-midautumn"><div align="center">Overall</div></td>
  </tr>
<tr>
	<%current_date=cdate(from_time)
	end_date=cdate(dateadd("d",-1,to_time))
...
for i=0 to ubound(a_series_names)
	for j=0 to ubound(a_line_names)
	SQL="select * from DAILY_SERIESLINE_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and FACTORY_ID='"&factory_id&"' and SERIES_NAME='"&a_series_names(i)&"' and LINE_NAME='"&a_line_names(j)&"' and LINE_DAY>=to_date('"&from_time&"','yyyy-mm-dd hh24:mi:ss') and LINE_DAY<=to_date('"&to_time&"','yyyy-mm-dd hh24:mi:ss')"
	rs.open SQL,conn,1,3
	if not rs.eof then
...
<tr>
  <td height="20"><div align="center"><%=i+1%></div></td>
    <td><div align="center"><span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyLineYieldDetail_Series.asp?factory_id=<%=factory_id%>&series_name=<%=a_series_names(i)%>&line_name=<%=a_line_names(j)%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><%=a_series_names(i)%></span></div></td>
    <td height="20"><div align="center"><%=a_line_names(j)%></div></td>
	<%
	current_date=cdate(from_time)
	end_date=cdate(dateadd("d",-1,to_time))
	do while current_date<=end_date
	SQL1="select * from DAILY_SERIESLINE_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and  FACTORY_ID='"&factory_id&"' and SERIES_NAME='"&a_series_names(i)&"' and LINE_NAME='"&a_line_names(j)&"' and to_char(LINE_DAY,'yyyy-mm-dd')='"&formatdate(current_date,application("F_shortdateformat"))&"'"
	...
	family_target_firstyield=csng(rs1("SERIES_TARGET_FIRSTYIELD"))
	else
	...
	<%if assembly_yield<>0 then%><span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyLineYieldDetail_Series.asp?factory_id=<%=factory_id%>&series_name=<%=a_series_names(i)%>&line_name=<%=a_line_names(j)%>&from_time=<%=current_date%>&to_time=<%=dateadd("d",1,current_date)%>')"><%=formatpercent(assembly_yield,2,-1)%></span>
	<%else%>
	...
	<span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyLineYieldDetail_Series.asp?factory_id=<%=factory_id%>&series_name=<%=a_series_names(i)%>&line_name=<%=a_line_names(j)%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><%=formatpercent(line_yield,2,-1)%></span>
	<%else%>
...
%>  
	...
	i=0
	do while current_date<=end_date
	SQL1="select * from DAILY_SERIESLINE_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and  FACTORY_ID='"&factory_id&"' and SERIES_NAME='OVERALL' and LINE_NAME='OVERALL' and to_char(LINE_DAY,'yyyy-mm-dd')='"&formatdate(current_date,application("F_shortdateformat"))&"'"
	rs1.open SQL1,conn,1,3
	if not rs1.eof then
