...
if request("Page_Source")="daily" then
from_time=request("from_time")
to_time=request("to_time")
else
from_time=request("fromdate")&" "&request("fromhour")&":"&request("fromminute")
to_time=request("todate")&" "&request("tohour")&":"&request("tominute")
end if
SQL="select * from JOB where FACTORY_ID='"&factory_id&"' and CLOSE_TIME>=to_date('"&from_time&"','yyyy-mm-dd hh24:mi:ss') and CLOSE_TIME<=to_date('"&to_time&"','yyyy-mm-dd hh24:mi:ss') order by JOB_NUMBER,SHEET_NUMBER"
'response.Write(SQL)
'response.End()
...
total_output_quantity=0
	...
	sheet.cells(1,1).value="Daily Line Yield Detail for "&factory_name&" ("&from_time&" - "&to_time&")"
	...
	sheet.cells(2,7).value="Close Time"
...
while not rs.eof
	...
	sheet.cells(i,5).value=rs("JOB_GOOD_QUANTITY")
	sheet.cells(i,6).value=formatpercent(csng(rs("JOB_ASSEMBLY_YIELD")),2,-1)
	sheet.cells(i,7).value=cstr(rs("CLOSE_TIME"))
