...
pagename="/Reports/Yield/DailyLineYieldYield/DailyLineYieldDetail_Series.asp"
...
<tr>
  <td height="20" colspan="9" class="t-c-greenCopy">Browse Recorded Daily Line Yield of <%=series_name%> -- <%=line_name%> (<%=from_time%> - <%=to_time%>) </td>
</tr>
<tr>
  <td height="20" colspan="9" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%">User:
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyLineYieldDetail_Export.asp?series_name=<%=series_name%>&line_name=<%=line_name%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td></tr>
<tr>
  <td height="20" colspan="9">Sort by: 
    <input name="ByJob" type="radio" value="1" onClick="javascript:window.open('DailyLineYieldDetailByJobNumber.asp?series_name=<%=series_name%>&factory_id=<%=factory_id%>&line_name=<%=line_name%>&from_time=<%=from_time%>&to_time=<%=to_time%>','_self')">
    Job Number
    <input name="ByPart" type="radio" value="1" onClick="javascript:window.open('DailyLineYieldDetailByPartNumber.asp?series_name=<%=series_name%>&factory_id=<%=factory_id%>&line_name=<%=line_name%>&from_time=<%=from_time%>&to_time=<%=to_time%>','_self')">
    Part Number </td>
</tr>
<tr>
  <td height="20" class="t-t-Borrow"><div align="center">NO</div></td>
  <%if DBA=true then%>
  <td class="t-t-Borrow"><div align="center">Update</div></td>
  <%end if%>
...
<tr>
  ...
  <%if DBA=true then%>
    <td>
      <div align="center">
        ...
        <input name="sheetnumber<%=i%>" type="hidden" id="sheetnumber<%=i%>" value="<%=rs("SHEET_NUMBER")%>">
        <input name="jobtype<%=i%>" type="hidden" id="jobtype<%=i%>" value="<%=rs("JOB_TYPE")%>">
  ...
  <%end if%>
    <td><div align="center"><span style="cursor:hand" class="d_link" onClick="javascript:window.open('/Job/SubJobs/JobDetail.asp?jobnumber=<%=rs("JOB_NUMBER")%>&sheetnumber=<%=rs("SHEET_NUMBER")%>&jobtype=<%=rs("JOB_TYPE")%>','_blank')"><%=rs("JOB_NUMBER")%>-<%=replace(rs("JOB_TYPE"),"N","")&repeatstring(rs("SHEET_NUMBER"),"0",3)%></span>&nbsp;
	 ...
	 <td><div align="center"><%= rs("CLOSE_TIME") %></div></td>
  </tr>
<%
