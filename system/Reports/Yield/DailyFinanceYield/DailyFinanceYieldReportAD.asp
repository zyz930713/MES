...
<tr>
  <td height="20" colspan="<%=cells%>" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="50%"><div align="right"><span style="cursor:hand" title="Regenerate Report" onClick="javascript:window.open('RegenerateDailyFinanceYieldReport.asp?profile_task_id=<%=profile_task_id%>&factory_id=<%=factory_id%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/IconRegenerate.gif" alt="Regenerate Report" width="22" height="22" align="absmiddle"></span>&nbsp;<span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyFinanceYieldReportAD_Export.asp?profile_task_id=<%=profile_task_id%>&factory_id=<%=factory_id%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/EXCEL_Middle.gif" alt="Export to Excel" width="22" height="22" align="absmiddle"></span>&nbsp;<span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyFinanceYieldSource_Export.asp?factory_id=<%=factory_id%>&from_time=<%=from_time%>&to_time=<%=to_time%>')">&nbsp;<img src="/Images/IconReportTable.gif" alt="Export the Source" width="22" height="22" align="absmiddle"></span></div></td>
      </tr>
    </table></td>
</tr>
<tr>
...
<tr>
  ...
  <td height="20" colspan="9" class="<%if weekday(current_date)=1 then%>t-b-Red<%elseif weekday(current_date)=7 then%>t-c-greenCopy<%else%>t-t-Borrow<%end if%>"><div align="center"><%=current_date%> (<%=shortweekdayconvert(weekday(current_date))%>)</div></td>
  <%current_date=dateadd("d",1,current_date)
	loop%>
	<td height="20" colspan="9" class="t-b-midautumn"><div align="center">Overall</div></td>
  </tr>
<tr>
	<%current_date=cdate(from_time)
	end_date=cdate(dateadd("d",-1,to_time))
  ...
  <td class="t-t-Borrow"><div align="center">Scarp Quantity </div></td>
...
<tr>
  <td height="20"><div align="center"><%=i+1%></div></td>
    <td><div align="center"><span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyFinanceYieldDetailAD.asp?factory_id=<%=factory_id%>&family_name=<%=a_family_names(i)%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><%=a_family_names(i)%></span></div></td>
    <%
	current_date=cdate(from_time)
	end_date=cdate(dateadd("d",-1,to_time))
	do while current_date<=end_date
    ...
    <td><div align="center"><%=output_quantity%></div></td>
	...
	<%if amount_yield<>0 then%><span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyFinanceYieldDetailAD.asp?factory_id=<%=factory_id%>&family_name=<%=a_family_names(i)%>&from_time=<%=current_date%>&to_time=<%=dateadd("d",1,current_date)%>')"><%=formatpercent(amount_yield,2,-1)%></span>
	<%else%>
	...
	<td><div align="center"><%=line_scrap_quantity%></div></td>
...
%>
	...
	<td><div align="center"><%=formatpercent(factory_plan_target,2,-1)%></div></td>
	<%
	current_date=dateadd("d",1,current_date)
	i=i+1
	loop
	...
	<td><div align="center"><%=overall_scrap_quantity%></div></td>
