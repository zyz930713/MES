...
rs.open SQL,conn,1,3
	...
	sheet.cells(2,3).value="Start Value"
	...
	sheet.cells(2,4).value="Output Value"
...
while not rs.eof
	...
	sheet.cells(i,3).value=rs("START_VALUE")
	sheet.cells(i,4).value=rs("GOOD_VALUE")
...
wend
	...
	sheet.cells(i,3).value=cstr(total_input_value)
	sheet.cells(i,4).value=cstr(total_output_value)
...
set Sheet=OExcel.activeWorkbook.ActiveSheet
	...
	sheet.cells(2,3).value="Scrap Value"
	sheet.cells(2,3).Interior.ColorIndex= xlColorDarkGray
	sheet.cells(2,3).Font.ColorIndex = xlColorWhite
	sheet.cells(2,3).Font.Bold = True
	sheet.cells(2,3).HorizontalAlignment = xlCenter
...
while not rs.eof
	sheet.cells(i,1).value=rs("PART_NUMBER")
	sheet.cells(i,2).value=rs("JOB_NUMBER")
	sheet.cells(i,3).value=rs("AMOUNT")
...
wend
	...
	sheet.cells(i,1).value="Total"
	sheet.cells(i,3).value=cstr(total_scrap_value)
	sheet.cells(i+1,1).value="Exported on "&now()
	sheet.range(sheet.cells(1,1),sheet.cells(1,3)).merge
	sheet.range(sheet.cells(i+1,1),sheet.cells(i+1,3)).merge
	set selection=sheet.range(sheet.cells(1,1),sheet.cells(i+1,3))
	selection.select
    With selection.Borders(xlEdgeLeft)
        .LineStyle = xlContinuous
        .Weight = xlThin
