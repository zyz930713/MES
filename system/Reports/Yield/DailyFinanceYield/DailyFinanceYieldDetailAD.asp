...
<tr>
  <td height="20" colspan="14" class="t-c-greenCopy">Browse Recorded Daily Finance Yield of <%=family_name%> -- <%=line_name%> (<%=from_time%> - <%=to_time%>) </td>
</tr>
<tr>
  <td height="20" colspan="14" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%">User:
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('DailyFinanceYieldDetailAD_Export.asp?family_name=<%=family_name%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td></tr>
<tbody id="FG">
<%SQL="select PART_NUMBER,JOB_NUMBER,START_QUANTITY,START_VALUE,GOOD_QUANTITY,GOOD_VALUE,TWIN,TRANSACTION_DATE FROM BAR_REPORT.STOCK_TRANSACTION WHERE FINANCE_FAMILY_ID='"&id&"' and TRANSACTION_DATE>=to_date('"&from_time&"','yyyy-mm-dd hh24:mi:ss') and TRANSACTION_DATE<to_date('"&to_time&"','yyyy-mm-dd hh24:mi:ss')"
session("SQL")=SQL
rs.open SQL,conn,1,3%>
<tr>
  <td height="20" colspan="8" class="t-t-Borrow">Output Detail </td>
  </tr>
<tr>
  <td height="20" class="t-t-Borrow"><div align="center">NO</div></td>
  <td class="t-t-Borrow"><div align="center">Part Number</div></td>
  <td class="t-t-Borrow"><div align="center">Job Number</div></td>
  <td class="t-t-Borrow"><div align="center">Start Quantity</div></td>
...
<tr <%if rs("TWIN")="1" then%>class="t-b-Yellow"<%end if%>>
  <td height="20"><div align="center"><% =i%></div></td>
    <td><div align="center"><%= rs("PART_NUMBER")%>&nbsp;</div></td>
	 ...
	 <td><div align="center"><%= rs("GOOD_QUANTITY")%></div></td>
	 <td><div align="center"><%= rs("GOOD_VALUE")%></div></td>
	 <td><div align="center"><%= rs("TRANSACTION_DATE")%></div></td>
    </tr>
<%
if rs("TWIN")="1" then
twin_quantity=rs("GOOD_QUANTITY")
twin_amount=rs("GOOD_VALUE")
else
twin_quantity=0
twin_amount=0
end if
total_start_quantity=total_start_quantity+ccur(rs("START_QUANTITY"))-twin_quantity
total_start_value=total_start_value+ccur(rs("START_VALUE"))-twin_amount
total_good_quantity=total_good_quantity+ccur(rs("GOOD_QUANTITY"))-twin_quantity
...
<tr class="t-c-GrayLight">
  ...
  <td>&nbsp;</td>
  <td>&nbsp;</td>
  <td><div align="center"><%= total_start_quantity%></div></td>
  <td><div align="center"><%= total_start_value%></div></td>
  <td><div align="center"><%= total_good_quantity%></div></td>
...
rs.open SQL,conn,1,3%>
<tr>
  <td height="20" colspan="7" class="t-t-Borrow">Scrap Detail </td>
  </tr>
<tr>
  <td height="20" class="t-t-Borrow"><div align="center">NO</div></td>
  <td class="t-t-Borrow"><div align="center">Part Number</div></td>
...
<tr class="t-c-GrayLight">
  ...
  <td>&nbsp;</td>
  <td>&nbsp;</td>
  <td><div align="center"><%= total_bad_quantity%></div></td>
