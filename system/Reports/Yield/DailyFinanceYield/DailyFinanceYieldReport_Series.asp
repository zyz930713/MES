...
pagename="/Reports/Yield/DailyStockYield/DailyStockYieldReport_Series.asp"
...
for i=0 to ubound(a_series_names)
	...
	SQL="select * from DAILY_SERIESSTOCK_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and  FACTORY_ID='"&factory_id&"' and SERIES_NAME='"&a_series_names(i)&"' and LINE_NAME='"&a_line_names(j)&"' and LINE_DAY>=to_date('"&from_time&"','yyyy-mm-dd hh24:mi:ss') and LINE_DAY<=to_date('"&to_time&"','yyyy-mm-dd hh24:mi:ss')"
	rs.open SQL,conn,1,3
	if not rs.eof then
...
<tr>
  <td height="20"><div align="center"><%=i+1%></div></td>
    <td><div align="center"><span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyStockYieldDetail_Series.asp?factory_id=<%=factory_id%>&series_name=<%=a_series_names(i)%>&line_name=<%=a_line_names(j)%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><%=a_series_names(i)%></span></div></td>
    <td height="20"><div align="center"><%=a_line_names(j)%></div></td>
	<%
	current_date=cdate(from_time)
	end_date=cdate(dateadd("d",-1,to_time))
	do while current_date<=end_date
	SQL1="select * from DAILY_SERIESSTOCK_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and  FACTORY_ID='"&factory_id&"' and SERIES_NAME='"&a_series_names(i)&"' and LINE_NAME='"&a_line_names(j)&"' and to_char(LINE_DAY,'yyyy-mm-dd')='"&formatdate(current_date,application("F_shortdateformat"))&"'"
	...
	family_internal_yield=csng(rs1("SERIES_TARGET_INTERNALYIELD"))
	family_target_yield=csng(rs1("SERIES_TARGET_YIELD"))
	else
	...
	<%if stock_yield<>0 then%><span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyStockYieldDetail_Series.asp?factory_id=<%=factory_id%>&series_name=<%=a_series_names(i)%>&line_name=<%=a_line_names(j)%>&from_time=<%=current_date%>&to_time=<%=dateadd("d",1,current_date)%>')"><%=formatpercent(stock_yield,2,-1)%></span>
	<%else%>
	...
	<span class="d_link" style="cursor:hand" onClick="javascript:window.open('DailyStockYieldDetail_Series.asp?factory_id=<%=factory_id%>&series_name=<%=a_series_names(i)%>&line_name=<%=a_line_names(j)%>&from_time=<%=from_time%>&to_time=<%=to_time%>')"><%=formatpercent(line_yield,2,-1)%></span>
	<%else%>
...
%>  
	...
	i=0
	do while current_date<=end_date
	SQL1="select * from DAILY_SERIESSTOCK_YIELD where PROFILE_TASK_ID='"&profile_task_id&"' and  FACTORY_ID='"&factory_id&"' and SERIES_NAME='OVERALL' and LINE_NAME='OVERALL' and to_char(LINE_DAY,'yyyy-mm-dd')='"&formatdate(current_date,application("F_shortdateformat"))&"'"
	rs1.open SQL1,conn,1,3
	if not rs1.eof then
