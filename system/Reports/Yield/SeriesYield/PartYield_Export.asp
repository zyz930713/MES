...
sheet.cells(1,8).value="Target Yield"
sheet.cells(1,9).value="Actual Yield"
...
while not rs.eof
	...
	sheet.cells(k,2).value=current_part_tag
	...
	sheet.cells(k,8).value=this_target_yield
	if csng(final_assembly_yield)<csng(this_target_yield)/100 then
	sheet.cells(k,8).Font.ColorIndex = xlColorRed
	end if
	sheet.cells(k,9).value=formatpercent(final_assembly_yield,2)
	k=k+1
	...
	sheet.cells(k,8).value=""
	sheet.cells(k,9).value=formatpercent(thisjob_assembly_yield(u),2)
	k=k+1
	next
