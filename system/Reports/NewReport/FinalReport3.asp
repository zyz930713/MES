...
if action="GenereateReport" then
		...
		SQL=SQL+"(select decode(sum(aa.goodqty),null,0,sum(aa.goodqty)) as goodqty ,decode(sum(bb.rejectqty),null,0,sum(bb.rejectqty)) as rejectqty ,"
		SQL=SQL+" bb.part_number_tag,aa.store_type from ( "
		SQL=SQL+" select sum(a.store_quantity) as goodqty,a.part_number_tag,a.store_type ,A.NID  from job_master_store a "
		...
		SQL=SQL+" group by part_number_tag,store_type ,A.NID) aa,"

		SQL=SQL+" (select sum(b.scrap_quantity) as rejectqty,b.part_number_tag ,B.STORE_NID from job_master_scrap b "
		...
		SQL=SQL+" group by part_number_tag ,B.STORE_NID) bb"
		SQL=SQL+" where bb.part_number_tag=aa.part_number_tag(+) AND bb.STORE_NID=aa.NID(+) group by bb.part_number_tag, store_type) temp"
...
<script>
	...
	{
		form1.action="FinalReport3.asp?Action=LoadNextItem"
		form1.submit();
	}
	...
	function GenerateReport()
	{
		...
		form1.action="FinalReport3.asp?Action=GenereateReport"
		form1.submit();
	}
...
<body>
<form action="FinalReport3.asp" method="post" name="form1" target="_self" id="form1">
