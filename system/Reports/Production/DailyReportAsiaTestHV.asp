...
mailContents = htmls & TitelTableHtml & TangoHtmlStr & AutoLineHtmlStr & OemLineHtmlStr & htmlDesc

MailSwitch = 0		'邮件发送/测试用显示开关
SendMailSub = request("SendMailSub")
if SendMailSub = "确认发送邮件" then MailSwitch = 3
if MailSwitch > 0 then
	select case MailSwitch
			...
			case else
				SendMailStr = "<center><form name='form' method='post' action=''><input type='submit' name='SendMailSub' value='确认发送邮件'/></form></center>"
				response.Write mailContents & SendMailStr & "</body></html>"
	end select
%>
...
<%
	SendJMail application("MailSender"),mailTo,"Daily Report KEB - " & mailTitleFromDate,mailContents & "</body></html>"
else
	SendMailStr = "<center><form name='form' method='post' action=''><input type='submit' name='SendMailSub' value='确认发送邮件'/></form></center>"
	response.Write mailContents & SendMailStr & "</body></html>"
end if
