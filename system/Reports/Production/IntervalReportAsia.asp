...
table th{border-color:black;font-size:14px;}
...
<form name="form1" method="post" action="IntervalReportAsia.asp">
From:<input name="StartDate" type="text" id="StartDate" value="<%=StartDate%>" size="10"> 07:15:00
      <script language=JavaScript type=text/javascript>
	function calendar3Callback(date, month, year)
	{
	document.all.StartDate.value=year + '-' + month + '-' + date
	}
    calendar3 = new dynCalendar('calendar3', 'calendar3Callback');
                        </script>
&nbsp;
To:<input name="EndDate" type="text" id="EndDate" value="<%=EndDate%>" size="10"> 07:15:00
<script language=JavaScript type=text/javascript>
	function calendar4Callback(date, month, year)
	{
	document.all.EndDate.value=year + '-' + month + '-' + date
	}
...
reportTime=replace(StartDate,"/","-")
fromTime=formatdate(reportTime,"dd.mm.yyyy")&" 07:15"
toTime=formatdate(EndDate,"dd.mm.yyyy")&" 07:14"
...
htmls=htmls&"<tr><td width='20%' rowspan='3'>&nbsp;</td>"
htmls=htmls&"<td colspan='3' align='center'><span style='height:40px;line-height:40px;font-size:24px;'><b>Interval Production Report</b></span></td>"
htmls=htmls&"<td width='20%' rowspan='3' align='right'><img src='http://"&application("HostServer")&"/Images/rpt-logo1.jpg' /></td></tr>"
htmls=htmls&"<tr><td width='23%' height='20%'><b>Author:&nbsp;</b>Zhang Hui, Wang Binglin</td>"
htmls=htmls&"<td width='14%'><b>Print Date:&nbsp;</b>"&formatdate(now(),"dd.mm.yyyy")&"</td>"
htmls=htmls&"<td width='23%'><b>Time:&nbsp;</b> "&fromTime&" - "&toTime&"</td></tr>"
htmls=htmls&"<tr><td colspan='3'>&nbsp;</td></tr></table></td></tr></table></br>"
...
htmlDesc=htmlDesc&"<tr><td>TTY</td><td>Total Yield, (Job Close Quantity / Job Open Quantity) last day 24H completed all JO yield</td></tr>"
...
OemAll = "'16mm MFD (KEB)','16mm MFD (KEW)','16mm MFD (797)','Julia-HAC','Julia-SC','Nith','Donau Slim (797)','Donau Slim (KEB)'"
...
mailContents = htmls & TitelTableHtml & TangoHtmlStr & AutoLineHtmlStr & OemLineHtmlStr & htmlDesc & "</div></body></html>"
...
<%
	SendJMail application("MailSender"),mailTo,"BPS Daily Report - " & mailTitleFromDate,mailContents
else
	response.Write(mailContents)
end if
...
function TitelTable(TitelName)
	...
	TotalTitel=TotalTitel&"<th rowspan='2' width='6%'>Actual<br>[pcs]</th><th rowspan='2' width='6%'>Real to<br>W/H</th>"
...
function ProdTypeTable(ProdType)
	TypeTitel="<table width='100%' border='1' cellpadding='0' cellspacing='0' ><tr align='center' bgcolor='#DCE6F1'><th colspan='15'>"&ProdType&"</th></tr>"
	...
	TypeTitel=TypeTitel&"<th rowspan='2' width='6%'>Actual<br>[pcs]</th><th rowspan='2' width='6%'>Real to<br>W/H</th>"
	...
	if not RsL.eof then
		do while not RsL.eof
			...
			elseif ProductName = "Beijing" then
				...
				if not RsTemp.eof then LineIdStr = RsTemp("LineId")
				RsTemp.close
				set RsTemp = nothing
					...
					LineTy = rs("line_type")
						...
						if (Cint(Lines1)/cint(Lines)) > 1 then
							...
							elseif LineTy = "OEM" then
								SqlStrT = "select product,COUNT(product) Lines,'Total' line_num,avg(pq_target) pq_target,(sum(nvl(pq_out,0))/sum(nvl(pq_start,1)+nvl(PQ_OFFSET,0)))*100 pq_yield,avg(ams_target) ams_target,(nvl(sum(AMS_GOOD),0))/(nvl(sum(ams_start),1))*100 ams_yield,avg(cosmetic_target) cosmetic_target,(nvl(sum(cosmetic_good),0))/(nvl(sum(cosmetic_start),1))*100 cosmetic_yield,AVG(tty_target) tty_target,(nvl(sum(tty_close),0))/(nvl(sum(TTY_OPEN),1))*100 TTy_yield,nvl(sum(target),0) Target,nvl(sum(actual),0) Actual,nvl(sum(real_to_wh),0) real_to_wh,nvl(sum(pq_out),0) pq_out,nvl(sum(pq_start+PQ_OFFSET),0) pq_start,nvl(sum(AMS_GOOD),0) AMS_GOOD,nvl(sum(ams_start),0) ams_start,nvl(sum(cosmetic_good),0) cosmetic_good,nvl(SUM(cosmetic_start),0) cosmetic_start,nvl(sum(tty_close),0) tty_close,nvl(SUM(TTY_OPEN),0) TTY_OPEN from bps_dr_all where product in ('"&ProName&"') and LINE_NUM not in ('Total','8411','8412') and '"&StartDate&"' < create_time AND create_time < = '"&EndDate&"' group by product"
							else
								SqlStrT = "select product,COUNT(product) Lines,'Total' line_num,avg(pq_target) pq_target,(sum(nvl(pq_out,0))/sum(nvl(pq_start,1)+nvl(PQ_OFFSET,0)))*100 pq_yield,avg(ams_target) ams_target,decode(sum(nvl(ams_start,1)),0,0,((sum(nvl(AMS_GOOD,0)) + (sum(nvl(ams_start,1)) - sum(nvl(AMS_GOOD,0)))* (select AVG(acousitc_offset) from RPT_dAILY_TARGET where product in ('"&ProName&"') and line in (select LINE_NUM from vw_bps_dr_all where product in ('"&ProName&"'))) )/sum(nvl(ams_start,1))))*100 ams_yield,avg(cosmetic_target) cosmetic_target,(nvl(sum(cosmetic_good),0))/(nvl(sum(cosmetic_start),1))*100 cosmetic_yield,AVG(tty_target) tty_target,(nvl(sum(tty_close),0))/(nvl(sum(TTY_OPEN),1))*100 TTy_yield,nvl(sum(target),0) Target,nvl(sum(actual),0) Actual,nvl(sum(real_to_wh),0) real_to_wh,nvl(sum(pq_out),0) pq_out,nvl(sum(pq_start+PQ_OFFSET),0) pq_start,nvl(sum(AMS_GOOD),0) AMS_GOOD,nvl(sum(ams_start),0) ams_start,nvl(sum(cosmetic_good),0) cosmetic_good,nvl(SUM(cosmetic_start),0) cosmetic_start,nvl(sum(tty_close),0) tty_close,nvl(SUM(TTY_OPEN),0) TTY_OPEN from bps_dr_all where product in ('"&ProName&"') and LINE_NUM not in ('Total','8411','8412') and '"&StartDate&"' < create_time AND create_time < = '"&EndDate&"' group by product"
							end if
...
Function TotalLineHtml(Switch,LineType,BjLineName,OemLineName)
	if Switch = 1 then
		...
		for i = 1 to 2
			if i = 1 then
				SqlStr = "select 'Beijing' product,count(product) LINE_NUM,nvl(sum(target),0) target,nvl(sum(actual),0) actual,nvl(sum(real_to_wh),0) real_to_wh,nvl(sum(target*pq_target),0)/nvl(sum(target),1) pq_target,nvl(sum(target*AMS_target),0)/nvl(sum(target),1) AMS_target,nvl(sum(target*cosmetic_target)/sum(target),0) cosmetic_target,nvl(sum(target*TTY_target)/sum(target),0) TTY_target,nvl(sum(pq_out)/sum(pq_start+PQ_OFFSET)*100,0) pq_Yield,nvl(sum(ams_good)/sum(ams_start)*100,0) AMS_Yield,nvl(sum(cosmetic_good)/sum(cosmetic_start)*100,0) cosmetic_Yield,nvl(sum(tty_close)/sum(tty_open)*100,0) TTY_Yield from bps_dr_all where product in ("&BjLineName&") and LINE_NUM not in ('Total','8411','8412') and '"&StartDate&"' < create_time AND create_time < = '"&EndDate&"'"
			elseif i = 2 then
