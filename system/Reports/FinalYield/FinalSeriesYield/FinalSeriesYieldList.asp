...
pagename="/Reports/FinalYield/FinalSeriesYield/FinalSeriesYieldList.asp"
SQL="select FS.*,F.FACTORY_NAME,U.USER_NAME from FINAL_SERIESYIELD_LIST FS inner join FACTORY F on FS.FACTORY_ID=F.NID left join USERS U on FS.CREATOR_CODE=U.USER_CODE where FS.NID is not null "&where&order
session("SQL")=SQL
...
<form name="form1" method="post" action="/Reports/FinalYield/FinalSeriesYield/FinalSeriesYieldList.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="5" class="t-c-greenCopy"><span>Search Final Series Yield Reports </span></td>
  </tr>
  <tr>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#006600" bordercolordark="#FFFFFF">
  <%if admin=true then%>
  <form action="/Reports/FinalYield/FinalSeriesYield/GenerateFinalSeriesYield.asp" method="post" name="form2" target="_self" onSubmit="return form2check()">
  <tr>
  ...
  <tr>
    ...
    <td width="16%">Job Created Time</td>
    <td width="60%">From
...
&nbsp; <input name="jtohour" type="text" id="jtohour" value="00:00" size="5"></td>
  ...
  <tr>
    <td height="20" colspan="4"><input name="Generate" type="submit" class="t-b-Yellow" id="Generate" value="Generate Final Series Yield Report at once"></td>
    </tr>
  </form>
  <%end if%>
</table>
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="10" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="48%">Browse Final Series Yield Report </td>
...
%>
  <tr>
	...
	<%if admin=true then%>
    <td><div align="center"><%if session("code")=rs("CREATOR_CODE") then%><span class="red" style="cursor:hand" onClick="javascript:location.href='DeleteFinalSeriesYieldReport.asp?id=<%=rs("NID")%>&finalseries_name=<%=rs("FINAL_SERIESYIELD_NAME")%>&path=<%=path%>&query=<%=query%>'"><img src="/Images/IconDelete.gif"></span><%else%><img src="/Images/IconDelete_No.gif"><%end if%></div></td>
	<%end if%>
    <td height="20">
      <div align="center">
        <% =formatdate(rs("REPORT_TIME"),application("longdateformat"))%>
      </div></td>
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('FinalSeriesYieldReport.asp?finalseries_id=<%=rs("NID")%>&finalseries_name=<%=rs("FINAL_SERIESYIELD_NAME")%>&finalseries_report_time=<%=rs("REPORT_TIME")%>&from_time=<%=rs("FROM_TIME")%>&to_time=<%=rs("TO_TIME")%>&factory_id=<%=rs("FACTORY_ID")%>')"><%= rs("FINAL_SERIESYIELD_NAME") %></span>&nbsp;</div></td>
    <td><div align="center">
      <%if rs("CHART_WEEK")<>"" then%>
      <span class="red" style="cursor:hand" onClick="javascript:window.open('http://<%=application("ChartServer")%>/KES1_Barcode/FinalSeriesYieldChart<%if rs("BY_LINE")="1" then%>ByLine<%end if%>.asp?factory=<%=rs("FACTORY_ID")%>&yearnumber=<%=rs("CHART_YEAR")%>&fromww=1&toww=<%=datepart("ww",rs("TO_TIME"))%>&factory_id=<%=rs("FACTORY_ID")%>')"><img src="/Images/IconReportChart.gif" width="22" height="22"></span>
      <%else%>
      &nbsp;
      <%end if%>
    </div></td>
