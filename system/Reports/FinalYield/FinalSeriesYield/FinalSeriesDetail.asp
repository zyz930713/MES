...
series_name=trim(request.Form("series_name"))
...
pagename="/Reports/FinalYield/FinalSeriesYield/FinalSeriesDetail.asp"
if store_nids<>"" then
...
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy">Browse Recorded Series Yield -- <%=series_name%></td>
</tr>
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="50%">User:
          <% =session("User") %></td>
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('SeriesDetail_Export.asp?series_name=<%=series_name%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td></tr>
<form name="formSort" method="post" action="">
<tr>
  <td height="20" colspan="8">Sort by: 
    <input name="ByJob" type="radio" value="1" onClick="document.formSort.action='FinalSeriesDetailbyJobNumber.asp';document.all.store_nids.value='<%=store_nids%>';document.all.series_name.value='<%=series_name%>';document.formSort.submit()">
    Job Number
    <input name="ByPart" type="radio" value="1" onClick="document.formSort.action='FinalSeriesDetailbyPartNumber.asp';document.all.store_nids.value='<%=store_nids%>';document.all.series_name.value='<%=series_name%>';document.formSort.submit()">
    Part Number </td>
</tr>
