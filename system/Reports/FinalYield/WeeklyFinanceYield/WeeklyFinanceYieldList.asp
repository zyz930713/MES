...
order=" order by WF.REPORT_TIME desc"
else
order=" order by "&ordername&" "&ordertype
end if
where=""
if reportname<>"" then
where=where&" and WF.FINAL_FAMILYYIELD_NAME='"&reportname&"'"
end if
if fromdate<>"" then
where=where&" and WF.REPORT_TIME>=to_date('"&fromdate&"','yyyy-mm-dd')"
end if
if todate<>"" then
where=where&" and WF.REPORT_TIME<=to_date('"&todate&"','yyyy-mm-dd')"
end if
...
SQL="select WF.*,F.FACTORY_NAME,U.USER_NAME from BAR_REPORT.WEEKLY_FINANCE_YIELD_LIST WF inner join FACTORY F on WF.FACTORY_ID=F.NID left join USERS U on WF.CREATOR_CODE=U.USER_CODE where WF.NID is not null "&where&order
session("SQL")=SQL
...
<form name="form1" method="post" action="/Reports/FinalYield/WeeklyFinanceYield/WeeklyFinanceYieldList.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="5" class="t-c-greenCopy"><span>Search Finance Yield Reports </span></td>
  </tr>
  <tr>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#006600" bordercolordark="#FFFFFF">
  <%if admin=true then%>
  <form action="/Reports/FinalYield/WeeklyFinanceYield/GenerateWeeklyFinanceYield.asp" method="post" name="form2" target="_self" onSubmit="return form2check()">
  <tr>
  ...
  <tr>
    ...
    <td class="t-b-blue">Job Created Time</td>
    <td>From
...
&nbsp; <input name="jtohour" type="text" id="jtohour" value="00:00" size="5"></td>
  ...
  <tr>
    <td height="20" colspan="6"><input name="Generate" type="submit" class="t-b-Yellow" id="Generate" value="Generate Finance Report at once"></td>
    </tr>
  </form>
  <%end if%>
</table>
