...
&nbsp; <input name="jtohour" type="text" id="jtohour" value="00:00" size="5"></td>
  ...
  <tr>
    <td height="20" colspan="6"><input name="NoYield" type="button" class="finished" id="NoYield" value="Check the No Yield Stock" onClick="javascript:if(document.form2.factory.selectedIndex!=0){window.open('/Job/Store/StoreNoYield/StoreNoYield.asp?factory='+document.form2.factory.options[document.form2.factory.selectedIndex].value+'&fromdate='+document.form2.jfromdate.value+'&fromhour='+document.form2.jfromhour.value+'&todate='+document.form2.jtodate.value+'&tohour='+document.form2.jtohour.value,'_blank','');}else{alert('Please select factory!');}">
    <input name="Generate" type="submit" class="t-b-Yellow" id="Generate" value="Generate Final Family Yield Report at once" onClick="javascript:if(byline.checked){document.form2.action='/Reports/FinalYield/FinalFamilyYield/GenerateFinalFamilyYieldByLine.asp'}else{document.form2.action='/Reports/FinalYield/FinalFamilyYield/GenerateFinalFamilyYield.asp'}"></td>
    </tr>
  </form>
  <%end if%>
</table>
