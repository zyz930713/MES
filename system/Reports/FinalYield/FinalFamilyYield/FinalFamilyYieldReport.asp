...
pagename="/Reports/FinalYield/FinalFamilyYield/FinalFamilyYieldReport.asp"
SQL="select FACTORY_TARGET_YIELD,FACTORY_TARGET_FIRSTYIELD,FACTORY_TARGET_INSPECTYIELD,CHART_WEEK,CHART_YEAR,CHART_MONTH from FINAL_FAMILYYIELD_LIST where NID='"&finalfamily_id&"'"
...
factory_target_inspectyield=rs("FACTORY_TARGET_INSPECTYIELD")
...
rs.close
SQL="select Y.* from FINAL_FAMILYYIELD_DETAIL Y where Y.FINAL_FAMILYYIELD_ID='"&finalfamily_id&"' and FAMILY_NAME<>'OVERALL' "&order
session("SQL")=SQL
rs.open SQL,conn,1,3
%>
...
<tr>
  <td height="20" colspan="15" class="t-c-greenCopy">Browse Recorded Final Family Yields -- <%=finalfamily_name%> from <%=formatdate(from_time,application("longdateformat"))%> to <%=formatdate(to_time,application("longdateformat"))%> (<% =formatdate(finalfamily_report_time,application("longdateformat"))%>)</td>
</tr>
<tr>
  <td height="20" colspan="15" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="50%"><div align="right"><span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('FinalFamilyYieldReport_Export.asp?finalfamily_id=<%=finalfamily_id%>&finalfamily_name=<%=finalfamily_name%>&finalfamily_report_time=<%=finalfamily_report_time%>&from_time=<%=from_time%>&to_time=<%=to_time%>&factory_id=<%=factory_id%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td>
</tr>
<tr>
...
<tr <%if rs("OVERALL_EXCEPTION")="1" then%>class="today"<%end if%>>
  <td height="20"><div align="center"><% =(session("strpagenum")-1)*pagesize_s+i%></div></td>
    ...
    <td><div align="center"><%if csng(rs("ASSEMBLY_INPUT_QUANTITY"))>0 then%><span class="d_link" style="cursor:hand" onClick="javascript:window.open('http://<%=application("ChartServer")%>/KES1_Barcode/FamilyLineLostPareto.asp?factory_id=<%=factory_id%>&seriesgroup=<%=rs("FAMILY_NAME")%>&from_time=<%=from_time%>&to_time=<%=to_time%>','_blank')"><img src="/Images/IconReportPareto.gif" width="22" height="22" align="absmiddle"></span><%else%>&nbsp;<%end if%></div></td>
...
'total_inspect_output=total_inspect_output/total_assembly_input
...
<input name="family_name" type="hidden" value="">
  ...
  <tr>
    <td height="20" colspan="15"><table border="1" cellpadding="0" cellspacing="0" bordercolorlight="#333333" bordercolordark="#FFFFFF">
  ...
  <tr>
    <td height="20" colspan="15">Chart Report:
