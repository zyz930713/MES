...
rs.open SQL,conn,3,3
	...
	sheet.cells(1,1).value="Final Yield Report of "&finalfamily_name&" from "&formatdate(from_time,application("longdateformat"))&" to "&formatdate(to_time,application("longdateformat"))&" (generated on "&finalfamily_report_time&")"
	...
	sheet.cells(2,8).value="Retest Yield"
	...
	sheet.cells(2,9).value="Final Yield"
	...
	sheet.cells(2,10).value="Internal Yield"
	...
	sheet.cells(2,11).value="Target Yield"
...
while not rs.eof
	...
	sheet.cells(i,5).Interior.ColorIndex = GetFirstYieldExcelColor(rs("ASSEMBLY_OUTPUT_QUANTITY"),rs("ASSEMBLY_YIELD"),rs("FAMILY_TARGET_FIRSTYIELD"))
	sheet.cells(i,6).value=formatpercent(csng(rs("FAMILY_TARGET_FIRSTYIELD"))/100,2,-1)
if csng(rs("ASSEMBLY_YIELD"))<>0 then
	delta=csng(rs("ASSEMBLY_YIELD"))-csng(rs("FAMILY_TARGET_FIRSTYIELD"))/100
	else
	delta=0
	end if
	sheet.cells(i,7).value=formatpercent(delta,2,-1)
	sheet.cells(i,8).value=formatpercent(csng(rs("INSPECT_YIELD")),2,-1)
	sheet.cells(i,8).Interior.ColorIndex = GetYieldExcelColor(rs("INSPECT_YIELD"),rs("FAMILY_TARGET_INSPECTYIELD"))
	sheet.cells(i,9).value=formatpercent(csng(rs("FINAL_YIELD")),2,-1)
	sheet.cells(i,9).Interior.ColorIndex = GetYieldExcelColor(rs("FINAL_YIELD"),rs("FAMILY_TARGET_YIELD"))
	sheet.cells(i,10).value=formatpercent(csng(rs("FAMILY_TARGET_INTERNALYIELD")),2,-1)
	sheet.cells(i,11).value=formatpercent(csng(rs("FAMILY_TARGET_YIELD"))/100,2,-1)
	if csng(rs("FINAL_YIELD"))<>0 then
	delta=csng(rs("FINAL_YIELD"))-csng(rs("FAMILY_TARGET_YIELD"))/100
	else
	delta=0
	end if
	sheet.cells(i,12).value=formatpercent(delta,2,-1)
	...
	sheet.cells(i,12).Interior.ColorIndex=xlColorGray
	end if
i=i+1
rs.movenext
wend
...
rs.close
	...
	sheet.cells(i,5).Interior.ColorIndex=GetFirstYieldExcelColor(total_assembly_output,total_assembly_yield,factory_target_firstyield)
	...
	if csng(total_assembly_yield)<>0 then
	delta=csng(total_assembly_yield)-csng(factory_target_firstyield)/100
	else
	delta=0
	end if
	sheet.cells(i,7).value=formatpercent(delta,2,-1)
	sheet.cells(i,7).Interior.ColorIndex=xlColorRose
	sheet.cells(i,8).value=formatpercent(total_inspect_yield,2,-1)
	sheet.cells(i,8).Interior.ColorIndex=GetYieldExcelColor(total_inspect_yield,factory_target_inspectyield)
	sheet.cells(i,9).value=formatpercent(total_final_yield,2,-1)
	sheet.cells(i,9).Interior.ColorIndex=GetYieldExcelColor(total_final_yield,factory_target_yield)
	sheet.cells(i,10).Interior.ColorIndex=xlColorRose
	sheet.cells(i,11).value=formatpercent(csng(factory_target_yield)/100,2,-1)
	...
	sheet.cells(i,12).value=formatpercent(delta,2,-1)
	sheet.cells(i,12).Interior.ColorIndex=xlColorRose
	'Remind*3
	...
	sheet.range(sheet.cells(i+1,3),sheet.cells(i+1,12)).merge
	sheet.cells(i+2,2).Interior.ColorIndex=xlColorYellow
	sheet.cells(i+2,3).value="Means delta is between 0% to 3%."
	sheet.range(sheet.cells(i+2,3),sheet.cells(i+2,12)).merge
	...
	sheet.range(sheet.cells(i+3,3),sheet.cells(i+3,12)).merge
	...
	sheet.range(sheet.cells(i+7,1),sheet.cells(i+7,12)).merge
	set selection=sheet.range(sheet.cells(1,1),sheet.cells(i+7,12))
	selection.select
