...
pagename="/Reports/Tracking/JobSchedule/SchedulePartDetail.asp"
SQL="select 1,J.*,P.PART_NUMBER from JOB_SCHEDULE_DETAIL J inner join PART P on J.PART_NUMBER_ID=P.NID where J.SCHEDULE_ID='"&schedule_id&"' and J.PART_NUMBER_ID='"&part_id&"'"
session("SQL")=SQL
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy">Browse Job Schedule for <%=part_name%></td>
</tr>
<tr>
...
<tr>
  ...
  <%if admin=true then%>
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='EditSchedule.asp?schedule_id=<%'=rs("ROW_ID")%>&path=<%=path%>&query=<%=query%>'">Edit</span></div></td>
    <td><div align="center">
      <%if rs("STATUS")="1" then%>
      <span class="red" style="cursor:hand" onClick="javascript:location.href='DisableScheduleItem.asp?job_number=<%=rs("JOB_NUMBER")%>&path=<%=path%>&query=<%=query%>'">Enabled</span>
      <%else%>
      <span class="red" style="cursor:hand" onClick="javascript:location.href='EnableScheduleItem.asp?job_number=<%=rs("JOB_NUMBER")%>&path=<%=path%>&query=<%=query%>'">Disabled</span>
      <%end if%>
    </div></td>
	<%end if%>
