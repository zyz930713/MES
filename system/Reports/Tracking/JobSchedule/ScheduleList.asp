...
SQL="select * from JOB_SCHEDULE_LIST where NID is not null "&where&order
session("SQL")=SQL
...
<body>
      <form name="form1" method="post" action="/Reports/Tracking/JobSchedule/ScheduleList.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="7" class="t-c-greenCopy"><span>Search Job Schedule Reports </span></td>
  </tr>
  <tr>
    <td height="20"><span class="style1">Year</span> </td>
    <td height="20"><input name="yearindex" type="text" id="yearindex" value="<%=yearindex%>">      </td>
    <td>Week</td>
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="10" class="t-c-greenCopy">Browse Job Schedule </td>
</tr>
<tr>
  <td height="20" colspan="10" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="50%"><div align="right">
          <%if admin=true then%><input name="Generate" type="button" id="Generate" value="Import Job Scedule" onClick="javascript:window.open('ImportSchedule.asp')">
		  <%end if%>
        </div></td>
      </tr>
    </table></td>
...
<tr>
  ...
  <%if admin=true then%>
  <td class="t-t-Borrow"><div align="center">Eidt</div></td>
  ...
  <td class="t-t-Borrow"><div align="center"><img src="/Images/Accend.gif" alt="Click to accend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=SCHEDULE_NAME&ordertype=asc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'">Schedule Name<img src="/Images/Decend.gif" alt="Click to decend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=SCHEDULE_NAME&ordertype=desc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'"></div></td>
  <td class="t-t-Borrow"><div align="center">Job List
  </div>
...
<tr>
  ...
  <%if admin=true then%>
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='EditSchedule.asp?schedule_id=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>'">Edit</span></div></td>
    ...
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('Schedule.asp?schedule_id=<%=rs("NID")%>&schedule_name=<%=rs("SCHEDULE_NAME")%>&schedule_report_time=<%=rs("REPORT_TIME")%>')"><%= rs("SCHEDULE_NAME") %></span>&nbsp;</div></td>
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('ScheduleJobList.asp?schedule_id=<%=rs("NID")%>&schedule_name=<%=rs("SCHEDULE_NAME")%>&schedule_report_time=<%=rs("REPORT_TIME")%>')"><%= rs("SCHEDULE_NAME") %></span></div></td>
    <td><div align="center"><%= rs("CREATOR_CODE") %></div></td>
    <td><div align="center"><% =formatdate(rs("REPORT_TIME"),application("longdateformat"))%></div></td>
    <td><div align="center"><% =formatdate(rs("UPDATE_TIME"),application("longdateformat"))%></div></td>
</tr>
<%
i=i+1
rs.movenext
