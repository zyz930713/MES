...
pagepara="&schedule_id="&schedule_id&"&part_id="&part_id&"&start_time="&start_time
pagename="/Reports/Tracking/JobSchedule/ScheduleDetail.asp"
SQL="select 1,J.*,P.PART_NUMBER from JOB_SCHEDULE_DETAIL J inner join PART P on J.PART_NUMBER_ID=P.NID where J.SCHEDULE_ID='"&schedule_id&"' and J.PART_NUMBER_ID='"&part_id&"' and (extract(year from J.START_TIME)='"&year(start_time)&"' and extract(month from J.START_TIME)='"&month(start_time)&"' and extract(day from J.START_TIME)='"&day(start_time)&"')"
session("SQL")=SQL
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="7" class="t-c-greenCopy"><span>Search Job in Schedule Reports </span></td>
  </tr>
  <tr>
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="8" class="t-c-greenCopy">Browse Job Schedule </td>
</tr>
<tr>
...
<tr>
  ...
  <%if admin=true then%><td class="t-t-Borrow"><div align="center">Eidt</div></td>
...
<tr>
  <td height="20"><div align="center"><% =(session("strpagenum")-1)*pagesize_s+i%></div></td>
    <%if admin=true then%><td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='EditScheduleItem.asp?schedule_id=<%'=rs("ROW_ID")%>&path=<%=path%>&query=<%=query%>'">Edit</span></div></td>
    <td><div align="center"><%if rs("STATUS")="1" then%><span class="red" style="cursor:hand" onClick="javascript:location.href='DisableScheduleItem.asp?job_number=<%=rs("JOB_NUMBER")%>&path=<%=path%>&query=<%=query%>'">Enabled</span><%else%><span class="red" style="cursor:hand" onClick="javascript:location.href='EnableScheduleItem.asp?job_number=<%=rs("JOB_NUMBER")%>&path=<%=path%>&query=<%=query%>'">Disabled</span><%end if%></div></td>
	<%end if%>
    ...
    <td><div align="center"><% =formatdate(rs("COMPLETE_TIME"),application("longdateformat")) %></div></td>
  </tr>
<%
i=i+1
rs.movenext
