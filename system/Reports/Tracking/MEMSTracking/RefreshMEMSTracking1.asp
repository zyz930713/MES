...
rs.open SQL,conn,1,3
	...
	if goon=true then
	SQL="delete from JOB_MEMS_DETAIL where MEMS_ID='"&id&"'"
	rs.open SQL,conn,1,3
	SQL="insert into JOB_MEMS_DETAIL select MEMS_LOT,MEMS_QUANTITY,JOB_NUMBER,MACHINE_NUMBER,STATION_START_TIME,MEMS_PART,PART_NUMBER_ID,CURRENT_STATION_ID,'"&id&"','"&rnd_key&"','"&session("code")&"' from JOB_MEMS_TEMP where rnd_key='"&rnd_key&"' and CREATOR_CODE='"&session("code")&"'"
	rs.open SQL,conn,1,3
	
	
	'Create EXCEL file
	set rsU=server.CreateObject("adodb.recordset")
	SQL="select 1,DJM.MEMS_LOT,DJM.MACHINE_NUMBER,DJM.MEMS_PART from DISTINCT_JOB_MEMS_DETAIL DJM where DJM.MEMS_ID='"&id&"'"
	rs.Open SQL,conn,1,3
	if not rs.EOF then
	
	
	filePath=server.mappath("\Reports\Excel")
	MEMSfilePath=server.mappath("\Reports\Tracking\MEMSTracking\EXCELs")
	...
	sheet.cells(i,3).value=rs("MACHINE_NUMBER")
		...
		SQLU="select 1,JM.JOB_NUMBER,JM.STATION_START_TIME,P.PART_NUMBER,JM.CURRENT_STATION_ID,JM.MEMS_QUANTITY from JOB_MEMS_DETAIL JM inner join PART P on JM.PART_NUMBER_ID=P.NID where JM.MEMS_LOT='"&rs("MEMS_LOT")&"' and JM.MEMS_ID='"&id&"' and JM.MEMS_PART='"&rs("MEMS_PART")&"' and JM.MACHINE_NUMBER='"&rs("MACHINE_NUMBER")&"' order by JM.JOB_NUMBER"
	...
	word="Successfully update MEMS report of "&MEMS_name&" at "&formatdate(create_time,application("longdateformat"))&"."
	action="location.href='"&beforepath&"'"
	else
	word="MEMS report of "&MEMS_name&" is not existed!"
	action="window.close()"
end if
%>
