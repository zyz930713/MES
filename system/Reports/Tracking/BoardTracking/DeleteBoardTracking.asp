...
board_name=request.QueryString("board_name")
excelfilename=request.QueryString("excelfilename")
SQL="delete from JOB_BOARD_LIST where NID='"&id&"'"
rs.open SQL,conn,3,3
SQL="delete from JOB_BOARD_DETAIL where BOARD_ID='"&id&"'"
rs.open SQL,conn,3,3
	set myFs=server.createObject("scripting.FileSystemObject") 
	ICfilePath=server.mappath("\Reports\Tracking\BoardTracking\EXCELs")
	myfile=ICfilePath&"\"&filename 
	if myFs.FileExists(myfile)=true then
	myFs.DeleteFile myfile
	end if
	set myFs=nothing
word="Board Report of "&board_name&" is deleted!"
%>
