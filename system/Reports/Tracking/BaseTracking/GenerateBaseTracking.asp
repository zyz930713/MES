...
order=" order by DJB.BASE_LOT asc"
else
order=" order by "&ordername&" "&ordertype
end if
where=""
if memslot<>"" then
where=where&" and DJB.BASE_LOT like '%"&memslot&"%'"
end if
if memspart<>"" then
where=where&" and DJB.BASE_PART='"&memspart&"'"
end if
if fromdate="" then
...
SQL="select 1,JB.*,P.PART_NUMBER from JOB_BASE_RAW JB inner join PART P on JB.PART_NUMBER_ID=P.NID where JB.START_TIME>=TO_DATE('"&fromdate&"','YYYY-MM-DD') and JB.START_TIME<=TO_DATE('"&todate&"','YYYY-MM-DD')"
session("SQL")=SQL
...
<body>
 <form name="form1" method="post" action="/Reports/Tracking/BaseTracking/GenerateBaseTracking.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="7" class="t-c-greenCopy"><span>Search Base Tracking </span></td>
  </tr>
  <tr>
    <td height="20"><span class="style1">Base Lot Number</span> </td>
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="38" class="t-c-greenCopy">Browse Base Tracking form <% =formatdate(fromdate,application("shortdateformat"))%> to <% =formatdate(todate,application("shortdateformat"))%></td>
</tr>
<tr>
...
<tr>
  ...
  <td class="t-t-Borrow"><div align="center"><img src="/Images/Accend.gif" alt="Click to accend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=DJB.BASE_PART&ordertype=asc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'">Base Part Number<img src="/Images/Decend.gif" alt="Click to decend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=DJB.BASE_PART&ordertype=desc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'"> </div></td>
  <td class="t-t-Borrow" colspan="10"><div align="center">Base Lot Number</div></td>
  </tr>
<%
i=1
if not rs.eof then
...
wend
%>
<form name="form2" method="post" action="/Reports/Tracking/BaseTracking/SaveBaseTracking.asp">
  <tr>
...
&nbsp;</td>
  ...
  <tr>
    <td height="20" colspan="38">Report name:
      <input name="BASE_name" type="text" id="BASE_name">
