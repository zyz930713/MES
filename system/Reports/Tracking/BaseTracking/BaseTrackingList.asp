...
pagename="/Reports/Tracking/BaseTracking/BaseTrackingList.asp"
SQL="select * from JOB_BASE_LIST where NID is not null "&where&order
session("SQL")=SQL
...
<body>
<form name="form1" method="post" action="/Reports/Tracking/BaseTracking/BaseTrackingList.asp">
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="7" class="t-c-greenCopy"><span>Search Base Tracking Reports </span></td>
  </tr>
  <tr>
...
&nbsp; </td>
    <td><img src="/Images/Find.gif" alt="Search" width="27" height="21" align="absmiddle" style="cursor:hand" onClick="javascript:document.form1.submit()"></td>
	  ...
	  <table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <tr>
    <td height="20" colspan="9" class="t-c-greenCopy">Browse Base Tracking </td>
  </tr>
  <tr>
    <td height="20" colspan="9" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="50%"><div align="right">
              <%if admin=true then%>
              <input name="Generate" type="button" id="Generate" value="Generate Base Tracking Report At Once" onClick="javascript:location.href='GenerateBaseTracking.asp?path=<%=path%>&query=<%=query%>'">
              <%end if%>
          </div></td>
        </tr>
    </table></td>
...
%>
  <tr>
    ...
    <%if admin=true then%>
    <td><div align="center"><span class="red" style="cursor:hand" onClick="javascript:location.href='DeleteBaseTracking.asp?id=<%=rs("NID")%>&base_name=<%=rs("TRACKING_NAME")%>&excelfilename=<%=rs("EXCEL_FILENAME")%>&path=<%=path%>&query=<%=query%>'">Delete</span></div></td>
    <%end if%>
    <td height="20"><div align="center"><%= rs("WEEK_INDEX") %></div></td>
    <td><div align="center"><%= rs("YEAR_INDEX") %></div></td>
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:window.open('BaseTracking.asp?base_id=<%=rs("NID")%>&base_name=<%=rs("TRACKING_NAME")%>&base_report_time=<%=rs("REPORT_TIME")%>&fromdate=<%=rs("WEEK_START_DAY")%>&todate=<%=rs("WEEK_END_DAY")%>')"><%= rs("TRACKING_NAME") %></span>&nbsp;</div></td>
    ...
    <td><div align="center"><a href="/Reports/Tracking/BaseTracking/EXCELs/<%=rs("EXCEL_FILENAME")%>" target="_blank"><img src="/Images/EXCELs.gif" width="15" height="15" border="0"></a></div></td>
  </tr>
  <%
i=i+1
rs.movenext
