...
SQL="select PF.*,U.USER_NAME,U2.USER_NAME as ACTOR_NAME,F.FORM_NAME from PROFILE_FORM PF inner join USERS U on PF.USER_CODE=U.USER_CODE left join USERS U2 on PF.ACTOR_CODE=U2.USER_CODE inner join FORM F on PF.FORM_ID=F.NID where PF.USER_CODE='"&session("code")&"' "&where&" order by APPLY_TIME desc"
end if
