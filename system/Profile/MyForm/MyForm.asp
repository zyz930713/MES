...
SQL="select 1,PF.*,U.USER_NAME,U2.USER_NAME as ACTOR_NAME,F.FORM_NAME from PROFILE_FORM PF inner join USERS U on PF.USER_CODE=U.USER_CODE left join USERS U2 on PF.ACTOR_CODE=U2.USER_CODE inner join FORM F on PF.FORM_ID=F.NID where PF.NID is not null "&where&" order by APPLY_TIME desc"
else
SQL="select 1,PF.*,U.USER_NAME,U2.USER_NAME as ACTOR_NAME,F.FORM_NAME from PROFILE_FORM PF inner join USERS U on PF.USER_CODE=U.USER_CODE left join USERS U2 on PF.ACTOR_CODE=U2.USER_CODE inner join FORM F on PF.FORM_ID=F.NID where PF.USER_CODE='"&session("code")&"' "&where&" order by APPLY_TIME desc"
end if
rs.open SQL,conn,1,3
%>
<html>
<head>
<title><%=application("SystemName")%> - My Form</title>
...
<form name="form1" method="get" action="/Profile/MyForm/MyForm.asp" target="_self">
...
while not rs.eof%>
  <tr class="t-c-GrayLight">
    ...
    <td height="20" bordercolorlight="#000099"><div align="center" class="red"><%if rs("FORM_STATUS")="1" then%><span style="cursor:hand" onClick="javascript:window.open('EditMyForm.asp?id=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>','main')">
    <span style="cursor:hand" onClick="javascript:window.open('EditMyForm.asp?id=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>','main')"><img src="/Images/IconEdit.gif" alt="Click to edit"></span>
    <%else%><img src="/Images/IconEdit_No.gif"><%end if%></span></div></td>
    <td height="20" bordercolorlight="#000099" class="red"><div align="center"><%if rs("FORM_STATUS")="1" then%><span style="cursor:hand" onClick="javascript:if(confirm('Are you sure to delete this form?')){window.open('DeleteMyForm.asp?id=<%=rs("NID")%>&job_id=<%=rs("ORACLE_JOB_ID")%>&path=<%=path%>&query=<%=query%>','main')}"><img src="/Images/IconDelete.gif" alt="Click to delete"><%else%><img src="/Images/IconDelete_No.gif" width="16" height="20"><%end if%></span></div></td>
