...
SQL="select J.* from JOB_MASTER_BACKUP J inner join FACTORY F on J.FACTORY_ID=F.NID where J.JOB_NUMBER is not null "&where&order
session("SQL")=SQL
  ...
  <td height="20" colspan="18" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="50%"><div align="right">
		  ...
		  <%end if%>
          <span style="cursor:hand" title="Export Data to EXCEL File" onClick="javascript:window.open('Job_Export.asp?fromdate=<%=fromdate%>&todate=<%=todate%>')"><img src="/Images/EXCEL.gif" width="30" height="30"></span></div></td>
      </tr>
    </table></td>
</tr>
<tr>
...
<tr>
  ...
  <td class="t-t-Borrow"><div align="center"><span id="inner_Stock"></span></td>
	...
	<tr>
	  <td height="20"><div align="center"><%=i%></div></td>
		...
		<%
		  ...
		  percent=round((cint(rs("FINAL_GOOD_QUANTITY"))+cint(rs("FINAL_SCRAP_QUANTITY")))/cint(rs("START_QUANTITY")),4)
		...
		<td><div align="center"><a href="/Job/Scrap/JobScrap/JobScrapDetail.asp?jobnumber=<%=rs("JOB_NUMBER")%>" target="_blank"><%= rs("FINAL_SCRAP_QUANTITY")%></a></div></td>
		<td><div align="center"><%= cint(rs("START_QUANTITY"))-cint(rs("FINAL_GOOD_QUANTITY"))-cint(rs("FINAL_SCRAP_QUANTITY"))%></div></td>
		<td><div align="center">
		  <% =formatdate(rs("INPUT_TIME"),application("longdateformat"))%>
	    &nbsp;</div></td>
		<td><div align="center"><% =formatdate(rs("FIRST_STORE_TIME"),application("shortdateformat"))%>&nbsp;</div></td>
