...
if computer <> "" then
	...
	if rs.eof then
		rs.addnew
		rs("computer_name")=computer
		rs("printer_name")=printer
		rs("line_name")=line
		rs("PRODUCT")=PRODUCT
		rs.update
		word="Add data successfully."
		action="location.href='"&beforepath&"'"
	else
		word="Computer name of "&computer&" has existed, please input again."
		action=""
	end if
	rs.close		
end if
...
<form name="form1" method="post" action="" onSubmit="return formcheck()">
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#666666" bordercolordark="#FFFFFF">
    ...
    <tr>
      ...
      <td  height="20"><input name="computer" type="text" id="computer" value="<%=computer%>">      
    </tr>
    <tr> 
