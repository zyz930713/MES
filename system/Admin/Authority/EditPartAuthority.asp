...
FactoryRight "O."
SQL="select 1,P.*,F.FACTORY_NAME from PART P inner join FACTORY F on P.FACTORY_ID=F.NID where P.NID='"&id&"'"
...
<body>
<form action="/Admin/Authority/EditPartAuthority1.asp" method="post" name="form1" target="_self" onSubmit="return formcheck()">
<table width="760"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="2" class="t-c-greenCopy">Edit Authority of a Part   </td>
</tr>
<tr>
...
<tr>
  <td width="183" height="20"><div align="left">Part Name 
      <input name="partnumber" type="hidden" id="partnumber" value="<%=rs("PART_NUMBER")%>">
  </div></td>
    <td width="571" height="20">
      <div align="left"><%=rs("PART_NUMBER")%></div></td>
    </tr>
  <tr>
      ...
      <table  border="1" align="left" cellpadding="0" cellspacing="0" bordercolorlight="#73A2EE" bordercolordark="#FFFFFF">
        ...
        <tr>
          <td rowspan="7"><select name="fromitem" size="15" multiple id="fromitem">
			<%= getOperator(true,"OPTION",""," where CODE not in ('"&getPartCodes(rs("NID"),"','",idcount)&"')"&factorywhereoutsideand," order by CODE","","",opcount) %>
			</select></td>
          ...
          <td rowspan="7"><select name="toitem" size="15" multiple id="toitem">
			<%= getOperator(true,"OPTION",""," where CODE in ('"&getPartCodes(rs("NID"),"','",idcount)&"')"&factorywhereoutsideand," order by CODE","","",opcount) %>
			</select></td>
