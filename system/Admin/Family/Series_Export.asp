...
rs.open SQL,conn,1,3
	...
	sheet.cells(1,1).value="系列清单"
	sheet.cells(2,1).value="系列名"
...
while not rs.eof
	...
	sheet.cells(i,2).value=rs("TARGET_FIRSTYIELD")
	sheet.cells(i,3).value=rs("TARGET_INTERNALYIELD")
	sheet.cells(i,4).value=rs("TARGET_YIELD")
	if rs("INCLUDED_SYSTEM_ITEMS")<>"" then
	sheet.cells(i,5).value=cstr(rs("INCLUDED_SYSTEM_ITEMS"))
	sheet.cells(i,5).HorizontalAlignment = xlLeft
	sheet.cells(i,5).VerticalAlignment = xlCenter
	sheet.cells(i,5).NumberFormatLocal = "G"
	sheet.cells(i,5).WrapText = True
	end if
i=i+1
rs.movenext
wend
