  ...
  <td height="20"><div align="center">
    <table  border="1" align="left" cellpadding="0" cellspacing="0" bordercolorlight="#73A2EE" bordercolordark="#FFFFFF">
       <%
		...
		where=" and M.ITEM_NAME not in (select * from table(cast(CHAR_SPLIT('"&rs("INCLUDED_SYSTEM_ITEMS")&"',',') as CHAR_TABLE)))"&factorywhereoutsideand
		else
		where=" and M.ITEM_NAME not in (select * from table(cast(CHAR_SPLIT('"&excluded_models&"',',') as CHAR_TABLE)))"&factorywhereoutsideand
		end if
	  	orderModel=" order by M.ITEM_NAME"
	  	session("fromModel")=where
	  	session("orderModel")=orderModel
		strmodel=getModel("OPTION","",where&" and rownum<=100",orderModel,"",idcount)
      ...
      <tr>
        ...
        <td rowspan="7"><select name="toitem" size="10" multiple id="toitem">
          <%where=""
			if rs("INCLUDED_SYSTEM_ITEMS")<>"" then
			where=" and M.ITEM_NAME in (select * from table(cast(CHAR_SPLIT('"&rs("INCLUDED_SYSTEM_ITEMS")&"',',') as CHAR_TABLE)))"
			%>
          <%= getModel("OPTION","",where," order by M.ITEM_NAME","","") %>
          <%end if%></select> </td>
