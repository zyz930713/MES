...
SQL="select P.*,F.FACTORY_NAME from PART P inner join FACTORY F on P.FACTORY_ID=F.NID where P.NID='"&id&"'"
...
<form action="/Admin/Part/EditSubPart1.asp" method="post" name="form1" target="_self" onSubmit="return formcheck()">
<table width="760"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="2" class="t-c-greenCopy">Edit a   Sub Part</td>
</tr>
<tr>
...
<tr>
  ...
  <tr>
    <td height="20">Routine Type  <span class="red">*</span></td>
    <td height="20"><select name="routine_type" id="routine_type">
      <option value="">-- Select Type --</option>
      <option value="0" <%if rs("ROUTINE_TYPE")="0" then%>selected<%end if%>>Main</option>
      <option value="1" <%if rs("ROUTINE_TYPE")="1" then%>selected<%end if%>>Sub</option>
    </select></td>
  </tr>
  <tr>
    ...
    <td height="20"><select name="routine_purpose" id="routine_purpose">
      ...
      <option value="0" <%if rs("ROUTINE_PURPOSE")="0" then%>selected<%end if%>>None</option>
