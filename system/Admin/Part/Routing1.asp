...
<form action="/Admin/Part/Routing1.asp" method="get" name="form1" target="_self">
  ...
  <td height="20" colspan="15" class="t-c-greenCopy"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td width="50%"><span id="inner_User"></span>:
          <% =session("User") %></td>
      <td width="50%"><div align="right"><a href="/Admin/Part/AddRouting_New.asp?path=<%=path%>&query=<%=query%>" target="main" class="white"><span id="inner_Add"></span></a></div></td>
    </tr>
  </table></td>
</tr>
<tr>
...
<img src="/Images/IconCopy.gif" alt="Click to copy routing"></span>         </td>
         <td height="20" ><div align="center">
         <span class="red" style="cursor:hand" onClick="javascript:window.open('EditRouting.asp?id=<%=rs("NID")%>&path=<%=path%>&query=<%=query%>','main')"><img src="/Images/IconEdit.gif" alt="Click to edit"></span></div></td>
    
    <td height="20"><div align="center"><span class="red" style="cursor:hand" onClick="javascript:if (confirm('Are you sure to delete this part? If deleted, new job can not apply it forever. ')){window.open('DeleteRouting.asp?id=<%=rs("NID")%>&partnumber=<%=rs("PART_NUMBER")%>&path=<%=path%>&query=<%=query%>','main')}"><img src="/Images/IconDelete.gif" alt="Click to delete" width="16" height="20"></span></div></td> 
    ...
    <td nowrap><div align="center"><%= replace(rs("PART_RULE"),",",",<br>") %></div></td>
         ...
         <%elseif rs("PART_TYPE")="4" then%>Change Model
...
%>
  <tr>
    <td height="20" colspan="15" align="center">No Records&nbsp;</td>
  </tr>
<%end if
rs.close%>
</table>
