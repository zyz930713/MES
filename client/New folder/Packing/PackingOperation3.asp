...
sql="update job_pack_detail set customer='"&customer&"',remarks='"&remarks&"',customer_label_id='"&custId&"' where box_id ='"&boxId&"'"
		...
		case "3" 'check 2D code
			...
			if rsAsyn.eof then
				...
				'sql="select * from job  where job_number='"&rsAsyn("job_number")&"' and sheet_number='"&sheet_number&"' and STATUS='0' and CLOSE_TIME is null "
'				
'				rs.open sql,conn,1,3
'				if  rs.eof  then
'				
'				else
'			'	
'				'if STATUSA=rs("STATUS") then
'				member("error")="此工单("&job_number&")没有走完流程，请关闭此工单."
'				'end if
'				end if
'				rs.close
			   ...
			   rs.close
				...
				'elseif left(rsAsyn("job_number"),3)<>"RWK" then			
					'	member("error")="This Job("&rsAsyn("job_number")&") is not TKB, cannot do  packing.|扫描的二维码("&codeValue&")的工单不是正常（RWK)工单，不能做RWK包装."						
				end if
