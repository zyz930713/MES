		...
		case "1"'Package Save			
			...
			for i=1 to request.Form("txt_jobno").count
				...
				'	
					...
					'ipackqty=csng(ipackqty)-existPackQty				
...
'			strWeek=DatePart("WW",YMD)
							...
							IF packType="FG" then
								...
								if member("error")="" then
												...
												if boxSize="2200" then
														Traytime=int(txt_count/550)
														LastQty=txt_count-550*Traytime 
														qty=int(550)
													   '调用打印功能
														sql="SELECT PRINTER_NAME FROM COMPUTER_PRINTER_MAPPING WHERE COMPUTER_NAME='"+UCase(request.Form("computername"))+"'"						
														rsAsyn.open sql,conn,1,3						
		...
		case "3" 'check 2D code
			...
			'member("error")="This 2D Code("&codeValue&") dose not exist.|扫描的二维码("&codeLine&")不存在."
			conn.beginTrans
			...
			if member("error")="" and packType="FG"  then
			   ...
			   rs.close
				...
				elseif left(rsAsyn("job_number"),2)="KB" then
			
						member("error")="This Job("&rsAsyn("job_number")&") is not RWK, cannot do RWK packing.|扫描的二维码("&codeValue&")的工单不是正常（RWK)工单，不能做RKW包装."		
				end if
					...
					if result <> "" then
						...
						if boxId="" then
							...
							if rs.eof then
								...
								boxIdNO=cint(right(boxId,3))+500
								'if len(boxId)=14 then
								boxId=left(boxId,13)
								boxId=boxId&boxIdNO
								'else
								'boxId=left(boxId,14)
...
function checkTestResult(str2DCode,strPartNumber)
	...
	strSql="select TEST_ENVIRONMENT,testResult, testTime ,rn from (  "
	strSql=strSql+"select  b.TEST_ENVIRONMENT, a.testTime, testResult,ROW_NUMBER() over(PARTITION by TEST_ENVIRONMENT order by a.testTime desc) rn  "
	strSql=strSql+"from TEST_RESULT_SUMMARY a left join TEST_RESULT_MAIN_KEB b on(a.archive_id=b.ARCHIVE_ID) where a.serialnumber = '"&str2DCode&"'"
	strSql=strSql+") temp  where rn=1  order by testTime desc "
	
'select TEST_ENVIRONMENT,testResult, testTime ,rn from ( 
'select  b.TEST_ENVIRONMENT, a.testTime, testResult,ROW_NUMBER() over(PARTITION by TEST_ENVIRONMENT order by a.testTime desc) rn 
'from TEST_RESULT_SUMMARY a left join TEST_RESULT_MAIN_KEB b on(a.archive_id=b.ARCHIVE_ID) where a.serialnumber = 'DYD4331D343G353A3')
'temp  where rn=1  order by testTime desc

	
	
'	strSql="select testResult,TEST_ENVIRONMENT from  TEST_RESULT_SUMMARY a left join TEST_RESULT_MAIN_KEB b on(a.archive_id=b.ARCHIVE_ID) where serialnumber = '"&str2DCode&"' order by testTime desc" 
	...
	do while not rsTSD.eof 	
		...
		if  rsTSD("testResult")="PASS" then
		  ...
		  testName=testName&rsTSD("TEST_ENVIRONMENT")&","
		
       else		
		
		 testNameQ=testNameQ&rsTSD("TEST_ENVIRONMENT")&","
	...
	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='RWK' and test_name  in('"&replace(testNameQ,",","','")&"') "
   ...
   if alarmMsg="" then
    ...
    strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='RWK'"
    rs.open strSql,conn,1,3
	...
	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='RWK'and test_name not in('"&replace(testName,",","','")&"') "
