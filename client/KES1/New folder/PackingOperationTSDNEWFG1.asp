...
'sqlstr=" SELECT a.testResult,b.TEST_ENVIRONMENT FROM (SELECT archive_id,testResult FROM TEST_RESULT_SUMMARY  WHERE serialNumber='DYD4377CE8HG353AK') a INNER JOIN TEST_RESULT_MAIN_KEB b ON (a.archive_id = b.ARCHIVE_ID) "

'sqlstr="SELECT a.testResult,b.TEST_ENVIRONMENT FROM (SELECT archive_id,testResult FROM TEST_RESULT_SUMMARY WHERE serialNumber='"&str2DCode&"') a INNER JOIN TEST_RESULT_MAIN_KEB b ON a.archive_id = b.ARCHIVE_ID"
