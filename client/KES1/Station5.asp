...
<% =session("code") %>
  ...
  <tr>
	...
	<%
									...
									WW=DatePart("ww", Date())
									DD=DatePart("w", Date())
									QYY=DatePart("YYYY", Date())
									QWW=DatePart("ww", Date()-2)
									QDD=DatePart("w", Date()-2)
									today=right(YY,1)&WW&DD
									Qday=right(QYY,1)&QWW&QDD
 ...
 %>
      ...
      <input type="hidden" name="CODE_DATE690"  id="CODE_DATE690" value="<% =rs("CODE_DATE690")%>">
      <input type="hidden" name="today"  id="today" value="<% =today%>">
      <input type="hidden" name="Qday"  id="Qday" value="<% =Qday%>">
     <%  end if
  rs.close%>
...
function getNewStationId(Station_Id)
  ...
  %>
    ...
    <%end if%>
    <input name="Record2" id="Record2" type="button" value="请扫描物料信息"  onClick="ToHVMaterialNew();">
	&nbsp;</div>
	</td></tr>
</table>
</form>
<script language="javascript">
function ToHVMaterialNew()
{
	
	toUrl="Material_HV_Record.asp";
	...
	//	var jobValue=document.form1.txtSubJobList.value;
	 ...
	 var VERSIONNUMBER= mid(object.value,15,1);
	   ...
	   if (VERSIONNUMBER ==-1)
	   {
	    ...
	    var CODE_DATE690 =document.all.CODE_DATE690.value; 
		var today =document.all.today.value; 
		var Qday =document.all.Qday.value; 
	    var CODEDATE690= mid(object.value,3,4);
		 
	  // if (CODE_DATE690 =="")
//	   {
//		 if (CODEDATE690>Qday && CODEDATE690<=today)
//			
//			{
//			
//			}
...
//	   else
//	   {
//		  if ( CODE_DATE690.indexOf(CODEDATE690)==-1)
//				
//				{
//				alert("喷码时间不对");
//		object.style.background="#CC99FF";
