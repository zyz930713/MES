...
<table id="table1" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    <thead>
      ...
      <tr >
        ...
        <td><input name="txt_opcode" type="text" id="txt_opcode" size="17" /></td>
	  </tr>
	  <tr>
		<td height="29" align="right"> 计划<span class="red"> *</span></td>
        <td><select id="slt_plan" name="slt_plan"  size="2" >
			<option value=""></option>
            <%sql="select plan_id,plan_date,CUSTOMER_NAME,part_number,to_char(delivery_time,'mmddhh24mi') delivery_time from packing_plan where status in ('Initial','Wait') order by delivery_time,priority"
				rs.open sql,conn,1,3
				while not rs.eof 
					response.Write("<option value='"+rs("plan_id")+"'>"+rs("part_number")+"-"+rs("delivery_time")+"-"+rs("CUSTOMER_NAME")+"</option>")
					rs.movenext
				wend
				rs.close
				%>
