...
if len(asynid)>0 then	
	select case asynid
		...
		case "4" 'input box id to RMA
			...
			unitQty=0
			sql="select DELIVERY_ID,sum(packed_qty) as uniQty from job_pack_detail where box_id='"&boxId&"' group by DELIVERY_ID"
			rs.open sql,conn,1,3
			if rs.eof then
				errMsg="This box id("&boxId&") does not exist.|此箱号("&boxId&")不存在."
			elseif isnull(rs("DELIVERY_ID"))  then
				errMsg="此箱号没有出过货，不能做客退处理。"
			else
		...
		if boxIdList <> "" then
			...
			sql=sql+",DELIVERY_ID,VENDOR_RECEIVE_DATE,CONFIRM_USER,CONFIRM_REMARKS,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME,GET_JSQ,BOXIDSTATUS,WHREC_JSQ) SELECT BOX_ID,JOB_NUMBER,PART_NUMBER,PACKED_QTY,CUSTOMER,PACKED_USER,PACKED_TIME,"
			sql=sql+"REMARKS||' Split by "&opCode&" at '||sysdate as REMARKS,IS_SHIPPED,PALLET_ID,SHIPPED_USER,"
			sql=sql+"SHIPPED_TIME,CUSTOMER_LABEL_ID,PACKED_LINE,PROD_SHIFT,SUPPLIER,PLAN_ID,STACK_USER,STACK_TIME,DELIVERY_ID,VENDOR_RECEIVE_DATE,CONFIRM_USER,CONFIRM_REMARKS,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME,GET_JSQ,BOXIDSTATUS,WHREC_JSQ FROM JOB_PACK_DETAIL "
			sql=sql+strWhere
			'errMsg=sql
			...
			sql=sql+",DELIVERY_ID='',WHREC_USER='"&opCode&"',WHREC_TIME=sysdate,GET_USER='',GET_TIME='',GET_JSQ='',BOXIDSTATUS='MRB',WHREC_JSQ='' "+strWhere
		   conn.execute(sql)
	
			if left (boxIDQTY,1)="," then
			boxIDQTY=right(boxIDQTY,len(boxIDQTY)-1)
			end if
'
'
  			tArr = Split(boxIDQTY,",") 
'
			...
			box_idQTY=(tArr(i))  
			tArrBOXQty = Split(box_idQTY,"-")
			box_id=tArrBOXQty(0) 
			Qty=tArrBOXQty(1) 
			sql="insert into rma_list (RMA_ID,BOX_ID,PACKED_QTY,RMA_USER,RMA_TIME) values ('"&txt_RMANO&"','"&box_id&"','"&Qty&"','"&opCode&"',sysdate)"
			conn.execute(sql)
			Next
       end if
'			
