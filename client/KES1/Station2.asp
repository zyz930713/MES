...
if codevalid=true then
	...
	if rs.eof then'no such job, it is a new job.
		...
		if jobtype="R" or left(inputJob,3)="RWK" or left(inputJob,3)="TKB" then 'retest job
			...
			SQL="select a.quantity,b.station_name from print_newjob_history a,rework_printing b where b.job_number='"&jobnumber&"' and  b.seq='"&sheetnumber&"' and a.new_jobnumber = '"&inputJob&"' "
	   ...
	   '----------------------------   一颗料号可以在手工线和自动线两个Routing中生产
		' SQLP=SQLP+" and decode(b.part_type,'0','N','1','R','5','S','4','C')='"+jobtype+"' and b.IS_DELETE='0' and A.PART_RULE like '%"&partnumber&"%' and a.LINES_INDEX like '%"&line_id&"%' order by a.MEET_PRIORITY, a.PART_RULE desc"
				 ...
				 '------------------------      料号最后几位可以用*号代替
				 SQLP=SQLP+" and decode(b.part_type,'0','N','1','R','5','S','4','C')='"+jobtype+"' and b.IS_DELETE='0' and a.LINES_INDEX like '%"&line_id&"%' order by a.MEET_PRIORITY, a.PART_RULE desc"
