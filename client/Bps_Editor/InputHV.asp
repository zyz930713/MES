...
<%
	...
	if CellDel = "delete" then
		response.write "我要delete"
		response.end
	end if
...
<table width="900" border="1" cellspacing="0" cellpadding="0"><tr><td align="center" class="TitleLable"><p style="height:34px;line-height:34px;">Line Output</p></td></tr>
	<tr>
		...
		　<input type="Submit" name="SearchEdit" id="SearchEdit" value="refurbish" />
		</td>
	</tr>
</table>
</br>
<%
	if SearchEdit = "refurbish" or SearchEdit = "ok" then
		...
		if IsNumeric(LineName) then
			...
			if not cellrs.eof then
				...
				if Egroup = "0" then
					DelSubmit = "<input id = 'CellDel' type='submit' name='CellDel' value='delete' style='background:red;' onClick='return beforeSubmit()'/>　　　"
				end if
			end if
			cellrs.close
			set cellrs = nothing
...
%>
			...
			<tr>
				...
				<strong>Product：<%=ProductName%>　Line：<%=LineName%>　Date：<%=Adate%>　D/N：<%=DName%>
				　Shift：
				...
				　<font color="red">JobNumber：<input type="text" name="JobNumber" id="JobNumber" value="<%=JobNumber%>" size="15"/>
				(Please input)</font>
				</strong>
				</td>
			</tr>
...
<table width="900" border="1" cellspacing="0" cellpadding="0">
	<tr>
		<td colspan="5" align="center">
				...
				<input type="button" value="Maintenance &amp; Test data" onClick="window.location.reload('HvMachine.asp');" />　　　
			</td>
		</tr>
	</table>
</form>
