...
if ShiftEditUser = "" then call sussLoctionHref("Connection timeout or not logged on","user_login.asp")
...
<%
	...
	if TestSub = "Modify" then
		...
		ConnSql.execute(Sqlstr)
		call sussLoctionHref("Modify","HvMachine.asp?SearchEdit=ok&ProductName="&ProductName&"&LineName="&LineName&"&Adate="&Adate&"&DName="&DName)
	end if
	'Modify线上数据信息'
	
	'Add设备维护信息'
	if MachineSub = "Add" then
	...
	if MachineSub = "Modify" then
		...
		ConnSql.execute(Sqlstr)
		call sussLoctionHref("Modify成功","HvMachine.asp?SearchEdit=ok&ProductName="&ProductName&"&LineName="&LineName&"&Adate="&Adate&"&DName="&DName)
	end if
	'Modify设备维护信息'
...
<table width="1260" border="1" cellspacing="0" cellpadding="0"><tr><td align="center" class="TitleLable"><p style="height:34px;line-height:34px;">Equipment repairing and data testing</p></td></tr>
		<tr>
			<td align="center">　Product：
			  <select name="ProductName" id="TmProduct"></select>
				　Line：
				...
				　Date：
				<input type="text" name="Adate" class="Wdate" id="Adate" onFocus="WdatePicker({isShowWeek:true,onpicked:function() {$dp.$('Adate_1').value=$dp.cal.getP('W','W');$dp.$('Adate_2').value=$dp.cal.getP('W','WW');}})" value="<%=Adate%>" size="10" />
				...
				<input type="submit" name="SearchEdit" value="refurbish">
			</td>
		</tr>
	</table>
</br>
<%
if SearchEdit = "refurbish" or SearchEdit = "ok" or Modi = "y" then
set rs = Server.CreateObject("adodb.recordset")
...
rs.open sql,ConnSql,1,1
    ...
    QABlock = 0
    QARelease = 0
	TestSubStr = "<input type='submit' name='TestSub' value='Add' style='background:#FFBFBF;'/>"
	if not rs.eof then
		OnlineId = rs("Rid")
		QtySM = rs("QtySM")
		QtyOC = rs("QtyOC")
		...
		TestSubStr = "<input type='submit' name='TestSub' value='Modify' style='background:#BFFFBF;'/>"
	end if
rs.close
%>
	<table cellpadding="1" border="1" cellspacing="0" id="table" width="1260">
		<tr>
			<td colspan="9">
				<strong>Product：<%=ProductName%>　Line：<%=LineName%>　Date：<%=Adate%>　D/N：<%=DName%></strong>
			</td>
		</tr>
		...
		<th>Online data:</th>
	        <th colspan="8" style="text-align:left;">
				...
				Sample：<input type="text" name="QtySM" value="<%=QtySM%>" size="4" />
				Scrap observation：
				<input type="text" name="QtyOC" value="<%=QtyOC%>" size="4" />
				Scrap clear：
				<input type="text" name="QtyTS" value="<%=QtyTS%>" size="4" />
				Membrane/Top Pot：
				<input type="text" name="QtyMT" value="<%=QtyMT%>" size="4" />
				Topping：
				<input type="text" name="QABlock" value="<%=QABlock%>" size="4" />　
				<%=TestSubStr%>
			</th>
		</tr>
		<tr><td colspan="9" height="5"></td></tr>
		<tr>
			<th width="130">maintenance</th>
			<th width="140">Start/End time</th>
			<th WIDTH="50">Time scope</th>
			<th WIDTH="60">Machine</th>
			<th width="90">Station</th>
			<th width="180">Error description</th>
			<th width="180">Error cause</th>
			<th width="180">Solving method</th>
	...
	rs.open sql,ConnSql,1,1
		...
		%>
			<tr>
				...
				<td><%=DReason%></td>
				<td><%=Dsolution%></td>
				<td><a href="HvMachine.asp?Bid=<%=Rid%>&ProductName=<%=ProductName%>&LineName=<%=LineName%>&Adate=<%=Adate%>&DName=<%=DName%>&Modi=y" >Modify</a></td>
			</tr>
		<%
	rs.movenext
	loop
...
'------------------------------------------------'	
	...
	if not rs.eof then
		...
		PcQty = rs("PcQty")
		PlanStop = rs("PlanStop")
		PcButton = "<input type='Submit' name='PCSub' value='Modify' style='background:#BFFFBF;'/>"
	else
		PcButton = "<input type='Submit' name='PCSub' value='Add' style='background:#FFBFBF;'/>"
	end if
	rs.close
	set rs = nothing
	...
	if Modi = "y" then
		...
		rs.close
		set rs = nothing
		AddMachineValue = "<input type='Submit' name='MachineSub' value='Modify' style='background:#BFFFBF;' onClick='return beforeSubmit()'/>"
	else
		AddMachineValue = "<input type='Submit' name='MachineSub' value='Add' style='background:#FFBFBF;' onClick='return beforeSubmit()'/>"
	end if
...
%>
		<tr>
			...
			<input type="hidden" id="BjId" name="MdId" value="<%=Bid%>" size="1" readonly>
				<select name="XlType">
					<option value="XL1" <%if MDlossn = "XL1" then response.write "selected" %> >replace spare parts</option>
					<option value="XL2" <%if MDlossn = "XL2" then response.write "selected" %> >maintain</option>
					<option value="XL3" <%if MDlossn = "XL3" then response.write "selected" %> >equipment adjustment</option>
					<option value="XL4" <%if MDlossn = "XL4" then response.write "selected" %> >planned detection </option>
					<option value="XL5" <%if MDlossn = "XL5" then response.write "selected" %> >Unit processing time</option>
					<option value="XL6" <%if MDlossn = "XL6" then response.write "selected" %> >velocity unmatched</option>
					<option value="XL7" <%if MDlossn = "XL7" then response.write "selected" %> >short pause</option>
					<option value="XL8" <%if MDlossn = "XL8" then response.write "selected" %> >processing waste</option>
					<option value="XL9" <%if MDlossn = "XL9" then response.write "selected" %> >original defective products</option>
				</select>
			</td>
			<td>
		...
		<tr>
			<td colspan="9">
				&nbsp;Total downtime:&nbsp;<font color="red"><%=SumDtime%></font>&nbsp;minute
				&nbsp;Expect downtime：
				<input type="text" name="PlanStop" style="text-align:center;" size="1" value="<%=PlanStop%>" />
				hour
				&nbsp;Online carriers number：
				<input type="text" name="PcSum" style="text-align:center;" size="1" value="<%=PcSum%>" />
				&nbsp;Waiting-for-repair carriers number：
				<input type="text" name="PcQty" style="text-align:center;" size="1" value="<%=PcQty%>" />
				&nbsp;<%=PcButton%>
		  </td>
		</tr>
...
<table width="1260" border="1" cellspacing="0" cellpadding="0">
	<tr>
		<td colspan="9" align="center">
				...
				<input type="button" value="Line output" onClick="window.location.reload('InputHV.asp');" />　　　　
...
function XlName(Xltype)
	select case Xltype
		case 1
			XlName = "replace spare parts"
		case 2
			XlName = "maintain"
		case 3
			XlName = "equipment adjustment"
		case 4
			XlName = "planned detection"
		case 5
			XlName = "Unit processing time"
		case 6
			XlName = "velocity unmatched"
		case 7
			XlName = "short pause"
		case 8
			XlName = "processing waste"
		case 9
			XlName = "original defective products"
	end select
end function
%>
</body>
