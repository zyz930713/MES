	 ...
	 function Print()
	 {
		  ...
		  if(document.form1.PRODTIMEC.value=="")
		{
		 ...
		 if (!document.form1.Equipment_NO.value)
			...
			form1.action="HV_Pot_print2.asp?Action=2";
			form1.submit();
		}
			...
			set rsAsyn=server.CreateObject("adodb.recordset")
				...
				boxIdN=boxId&"-"&i
					...
					if rtnCode<>"OK" then
				   	...
				   	rtnCode=PrintCtl.PrintKEBHVPOTLabel(printName,PRODUCTION_NAME,PART_NUMBER,boxId,Shift,pack_Time,qty,POT,TOP_Plate,Magent,Equipment_NO,PACKED_USER,InSpector,JOB_NUMBER,Remark,BATCHNOA,BATCHNOB,BATCHNOC)
...
sql="insert into EMR_PACK_DETAIL  (PACK_USER,PART_NUMBER,PRODUCTION_NAME,BOX_ID,SHIFT,PACK_TIME,PACK_QTY,SUPPLIERA,SUPPLIERB,SUPPLIERC,EQUIPMENT_NO,INSPECTOR,REMARK,JOB_NUMBER,BATCHNOA,BATCHNOB,BATCHNOC,PRODTIMEA,PRODTIMEB,PRODTIMEC,JSQ) VALUES  ('"+packed_USER+"','"+part_NUMBER+"','"+PRODUCTION_NAME+"','"+boxId+"','"+Shift+"','"&PACK_TIME&"','"+qty+"','"+SUPPLIERA+"','"+SUPPLIERB+"','"+SUPPLIERC+"','"+EQUIPMENT_NO+"','"+INSPECTOR+"','"+REMARK+"','"+JOB_NUMBER+"','"+BATCHNOA+"','"+BATCHNOB+"','"+BATCHNOC+"','"+PRODTIMEA+"','"+PRODTIMEB+"','"+PRODTIMEC+"','"+qty+"')"
...
<table width="1200" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
  <form id="form1" method="post" name="form1">  	
    <tr>
      <td height="20" colspan="6" class="t-t-DarkBlue"  align="center">(HV Pot Assy Print)</td>
    </tr>
    ...
    <tr>
      ...
      <td ><select name="Magent" id="Magent">
        <option value="ChaoYa">ChaoYa</option>
          ...
          <option value="OneMagent">OneMagent</option>    
	      <option value="NA">NA</option>
      </select></td>
