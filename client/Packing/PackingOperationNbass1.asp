									...
									if member("error")="" then
												...
												if YESNO_LITTLE_LABLE="YES"  then												
													   if isnull(SMALL_PACK) or  cstr(SMALL_PACK)=0  then
															...
															if clng(txt_count)=cint(boxSize) then
				    ...
				    elseif   clng(rsAsyn("JOB_GOOD_QTY"))<=0  then 
						...
						if member("error")="" and packType="FG" or PackType="SCRAP" then
							...
							if boxSize="" then
								...
								sql="select nvl(box_size,0) as box_size,CUSTOMER_DESC_EN,CUSTOMER_LABEL from CUSTOMER_MATERIAL where item_name='"&partNo&"'"
								'boxSize=0
								rs.open sql,conn,1,3
								if not rs.eof then
									boxSize=clng(rs("box_size"))
									CUSTOMER_DESC_EN=rs("CUSTOMER_DESC_EN")
									CUSTOMER_LABEL=rs("CUSTOMER_LABEL")
								else
