		...
		case "3" 'check 2D code
			...
			if member("error")="" and packType="FG"  then
			   ...
			   rs.close
				...
				elseif left(rsAsyn("job_number"),2)<>"RW" then
...
'	strSql="select  a.testResult,a.testitem from package a ,serial_index b where a.serial_id=b.serial_id and serialnumber = '"&str2DCode&"'" 
	...
	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='TSDRWK' and test_name  in('"&replace(testNameQ,",","','")&"') "
   ...
   if alarmMsg="" then
    ...
    strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='TSDRWK'"
    rs.open strSql,conn,1,3
	...
	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='TSDRWK'and test_name not in('"&replace(testName,",","','")&"') "
