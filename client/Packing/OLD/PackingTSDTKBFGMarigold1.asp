		...
		case "3" 'check 2D code
			...
			if member("error")="" and packType="FG"  then
			   ...
			   rs.close
				...
				elseif left(rsAsyn("job_number"),2)<>"TK" then
			
						member("error")="This Job("&rsAsyn("job_number")&") is not TKB, cannot do TKB packing.|扫描的二维码("&codeValue&")的工单不是正常（TKB)工单，不能做TKB包装."		
				end if
...
function checkTestResult(str2DCode,strPartNumber)
	...
	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='TSDTKB' and test_name  in('"&replace(testNameQ,",","','")&"') "
   ...
   if alarmMsg="" then
    ...
    strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='TSDTKB'"
    rs.open strSql,conn,1,3
	...
	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_type='TSDTKB'and test_name not in('"&replace(testName,",","','")&"') "
