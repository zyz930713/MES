...
<script type="text/javascript">
	...
	function Save(){
		...
		$.post("PackingOperationCheckABCS.asp",r,function(data,textStatus){
			//alert(data);
			var result=$.parseJSON(data);
			if(result.error==undefined){
				alert(result.message.replace("|","\n"));
				window.location="PackingOperationABCD.asp"; 
				$("#btnReset").click();	
				$("table tbody").html("");
				AddRows();
				document.getElementById("unitTestInfo").src="PVSTestCheckABC.asp";	
			}
			else{
	...
	function Print(){
		...
		$.ajax({
			type: "post",
			url: "PackingOperationCheckABCS.asp",
			async:false,
			data:r,
	...
	function input2DCode(objValue){
	  ...
	  if($("#txt_opcode").val().length==0){
		...
		document.getElementById("unitTestInfo").src="PVSTestCheckABC.asp?txt_2d_code="+name;	
...
if txt_boxid<>"" then
		   ...
		   rsAsyn.open sql,conn,1,3
			...
			response.Redirect("PackingOperationNPI.asp?word="&word)
			else
...
<input type="hidden" id="txt_pack_type" value="<%=packType%>" />
  ...
  <center>
  <iframe src="PVSTestCheckABC.asp" width="95%" height="320" scrolling="auto" frameborder="0" id="unitTestInfo"></iframe>
  </center>
