				...
				if member("error")="" then
						...
						elseif customerLabel="Tango" then
							...
							IF packType="FG" then
								...
								If member("error")="" then												
												if boxSize="0" then
												sql="select nvl(box_size,0) as box_size, nvl(SMALL_PACK,0) as SMALL_PACK,YESNOLITTLELABLE from product_model where item_name='"&partNo&"'"
												boxSize=0
												rs.open sql,conn,1,3
													if not rs.eof then
														boxSize=rs("box_size")
...
function checkTestResult(str2DCode,strPartNumber)
	...
	do while not rsPVS.eof 	
		...
		if  rsPVS("adfail")="False" then
		 ...
		 testNameQ=testNameQ&left(rsPVS("linename"),3)&","
