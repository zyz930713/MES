...
<script type="text/javascript">
	...
	function Save(){
		...
		$.post("PackingOperationTSDTKBFG1.asp",r,function(data,textStatus){
			//alert(data);
			var result=$.parseJSON(data);
			if(result.error==undefined){
				alert(result.message.replace("|","\n"));
				window.location="PackingOperationTSDTKBFG.asp"; 
				$("#btnReset").click();	
				$("table tbody").html("");
				AddRows();
	...
	function Print(){
		...
		$.ajax({
			type: "post",
			url: "PackingOperationTSDTKBFG1.asp",
			async:false,
			data:r,
	...
	function input2DCode(objValue){
	 ...
	 if (!$("#txt_boxsize").val()=="" &&  $("#txt_count").val()){
		...
		document.getElementById("unitTestInfo").src="TSDTestData.asp?txt_2d_code="+name;	
