...
if len(asynid)>0 then
	...
	select case asynid
		...
		case "2"'label print
			...
			rsTicket.close	
			if mid(job_number,1,3) ="RWK" then
			packLine=mid(job_number,4,4)			
			else						
			...
			if member("error")="" then
			   ...
			   'member("error")="工单号与成品料号不匹配，请不要乱输入工单号与料号。"				   
				...
				if member("error")="" then			
				'产生Customer label id
					sql="select CUSTOMER_PN,VENDOR_CODE,CUSTOMER_DESC_EN,REV,nvl(box_size,0) as box_size, nvl(SMALL_LABLE_SIZE,0) as SMALL_PACK,YESNO_LITTLE_LABLE,PRODUCT_TYPE from CUSTOMER_MATERIAL where item_name='"&partNo&"' and CUSTOMER_LABEL='"&CUSTOMER&"'"
					rs.open sql,conn,1,3
					if not rs.eof then
									...
									if member("error")="" then
												...
												if YESNO_LITTLE_LABLE="YES"  then												
													   if isnull(SMALL_PACK) or  cstr(SMALL_PACK)=0  then
															...
															if customer="Samsung" then	
															result=	PrintSamsungSMALLBoxLable(cstr(Qty),cstr(SMALL_PACK))	
...
'								 
'	end if		
					function PrintSamsungLable
								   
						
						
						tArr = Split(VENDOR_CODE,"/") 
						VENDOR_CODES=tArr(1)						
						boxIds=right(boxId,10)
					    Qtys=repeatstring(Qty,"0",6)
						
	           
					    custId=customerPN&VENDOR_CODES&REV&boxIds&Qtys
					...
					rsAsyn.open sql,conn,1,3
						...
						rtnCode=PrintCtl.PrintSamsungKEBCustomerLabel(printName,custId,customerPN,CUSTOMER_DESC_EN,REV,boxIds,cstr(Qtys),partNo,VENDOR_CODE)  
						rtnCode=PrintCtl.PrintSamsungKEBCustomerLabel(printName,custId,customerPN,CUSTOMER_DESC_EN,REV,boxIds,cstr(Qtys),partNo,VENDOR_CODE)  																	
						end if
			        ...
			        PrintSamsungLable=alarmMsg
													
					end function							

 			function PrintSamsungSMALLBoxLable (sumcount,pouchpack)
					
					Traytime=int(sumcount/pouchpack)
					if int(Traytime/3) < 1 then
					Traytimes=int(Traytime/3)+1
					else
						if csng(Traytime/3)>int(Traytime/3) then
						Traytimes=int(Traytime/3)+1
						end if
					end if
					'alarmMsg=cstr(Traytimes)					
				   ...
				   '调用打印功能
					...
					if printName="" then
						...
						set PrintCtl=server.createobject("PrintClass.PrintCtl")                  
						if Traytimes>0 then
						for i=1 to Traytimes 	
						  
								                                           			 
																				
						     rtnCode=PrintCtl.PrintSamsungKEBTrayLabel(printName,custId)
					...
					PrintSamsungSMALLBoxLable=alarmMsg
			   end function
						 ...
						 if member("error")="" then
								...
								if printName="" then
									...
									set PrintCtl=server.createobject("PrintClass.PrintCtl")                  
									    ...
									    rtnCode=PrintCtl.PrintOEMKEBBoxIDLabel(printName,boxid,customer,partNo,cstr(qty),remarks&" "&CUSTOMER_DESC_EN,INSPECTIONPQC)
					...
					if printName="" then
						...
						boxIdN=boxId&"-"&i 
							...
							rtnCode=PrintCtl.PrintHWCustomerLabel(printName,customer,DateCode,customerPN,cstr(qty),LotNO,boxid,Vendor_Code)
								elseif partNO="240326000031" or  partNO="240326300194" or   partNO="240326300207" then
