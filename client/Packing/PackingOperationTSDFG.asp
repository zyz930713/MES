...
<script type="text/javascript">
	...
	function Save(){
		...
		$.post("PackingOperationTSDFG1.asp",r,function(data,textStatus){
			//alert(data);
			var result=$.parseJSON(data);
			if(result.error==undefined){
				alert(result.message.replace("|","\n"));
				window.location="PackingOperationTSDFG.asp"; 
				$("#btnReset").click();	
				$("table tbody").html("");
				AddRows();
				document.getElementById("unitTestInfo").src="TSDTestData.asp";	
			}
			else{
	...
	function Print(){
		...
		$.ajax({
			type: "post",
			url: "PackingOperationTSDFG1.asp",
			async:false,
			data:r,
	...
	function input2DCode(objValue){
	 ...
	 if (!$("#txt_boxsize").val()=="" &&  $("#txt_count").val()){
		...
		document.getElementById("unitTestInfo").src="TSDTestData.asp?txt_2d_code="+name+"&PRODUCT="+form1.PRODUCT.value;;	
			...
			$.ajax({
				...
				success:function(data,textStatus){
					...
					if(result.error==undefined){
						...
						$("#txt_BOX_JOB_TYPE").val(result.BOX_JOB_TYPE);					
						$("#lab_OKT").val(result.OKT);
...
if txt_boxid<>"" then
	...
	if rsAsyn("SN")="0" then
	 ...
	 sqlCstm="select  box_id,customer_label_id,Remarks,BOX_JOB_TYPE  from job_pack_detail where  box_id='"&txt_boxid&"' group by box_id,customer_label_id,Remarks,BOX_JOB_TYPE"
			rs.open sqlCstm,conn,1,3
			if not rs.bof then			
			CUSTOMER_LABEL_ID=rs("CUSTOMER_LABEL_ID")
			Remarks =rs("Remarks")
			BOX_JOB_TYPE=rs("BOX_JOB_TYPE")
			end if
			rs.close
		
			'sqlBS="select BOX_SIZE from product_model where  ITEM_NAME='"&part_number_tag&"'"			
sqlBS="select nvl(box_size,0) as box_size from CUSTOMER_MATERIAL"
			sqlBS=sqlBS+" where item_name='"&part_number_tag&"' and CUSTOMER_LABEL='"&request("txt_customer")&"'"
...
<input type="hidden" id="txt_pack_type" value="<%=packType%>" />
  ...
  <table id="table1" width="800px" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    <thead>
      <tr>
        <td colspan="4" class="t-t-DarkBlue" align="center">Week Packing 包装
      ...
      <tr >
        ...
        <td><input type="text" id="txt_remarks" name="txt_remarks" style="background-color:#666666" readonly="true" value="<%=Remarks%>" /></td>
      </tr>
	  <tr  >
	  ...
	  <tr >
        ...
        <td><input type="text" id="txt_boxsize" name="txt_boxsize"  size="22" style="background-color:#666666" readonly="true" value="<%=BOX_SIZE%>" /></td>
	  ...
	  <tr >
	   ...
	   <tr >
        ...
        <td><input type="text" id="txt_custid" name="txt_custid" style="background-color:#666666" readonly="true"  value="<%=CUSTOMER_LABEL_ID%>"/></td>
	   </tr>
	   <tr>
	     <td align="right">包装工单类型	       </td>
	     <td align="left"><input name="txt_BOX_JOB_TYPE" type="text" id="txt_BOX_JOB_TYPE" style="background-color:#666666" value="<%=BOX_JOB_TYPE%>" size="22" readonly="readonly" /></td>
	     <td align="center">&nbsp;</td>
	     <td align="center">&nbsp;</td>
	   </tr>
	  <tr><td colspan="4" align="center"><input name="lab_OKT" type="text" id="lab_OKT" style="background-color:#666666"  value="" size="4" readonly="readonly"/></td></tr>
      <tr align="center">
  ...
  <center>
  <iframe src="TSDTestData.asp" width="95%" height="320" scrolling="auto" frameborder="0" id="unitTestInfo"></iframe>
  </center>
  <input type="hidden" id="computername" name="computername" />
  <input type="hidden" id="PRODUCT" name="PRODUCT"  value="<%=request("PRODUCT")%>"/>
</form>
<%'end if%>
</body>
</html>
