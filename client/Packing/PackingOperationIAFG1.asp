...
function checkTestResultKB(str2DCode,strPartNumber)
	...
	' sql="select [Pr_ID],[serialNumber],[testDateTime],[upDateTime],[pr_fail],[errorName],'INLINE2ND' as testitem from [TSD_EXPLORER].[tsd].[IA_List] where  serialnumber = '"&str2DCode&"'" 
	  sql="select [Pr_ID],[serialNumber],[testDateTime],[upDateTime],[pr_fail],[errorName],'INLINE2ND' as testitem from [TSD_EXPLORER].[tsd].[IA_List] where  serialnumber = '"&str2DCode&"'" 
...
'response.Write(sql)
	...
	while not rsIA.eof 	
		...
		if rsIA("pr_fail")="0" then
			testName=testName&rsIA("testitem")&","
...
'	rsIA.open strSql,connIA,1,3		
'	K=0
'	do while not rsIA.eof 	
'
'		if  rsIA("testResult")="PASS" then
'		
'		  testName=testName&rsIA("testitem")&","
'		
'       else		
'		
'		 testNameQ=testNameQ&rsIA("testitem")&","
'			
'	    end if
'	rsIA.movenext
'	loop
