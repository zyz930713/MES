...
<script type="text/javascript">
	...
	function Save(){
		...
		$.post("PackingOperationIAFG1.asp",r,function(data,textStatus){
			//alert(data);
			var result=$.parseJSON(data);
			if(result.error==undefined){
				alert(result.message.replace("|","\n"));
				window.location="PackingOperationIAFG.asp"; 
				$("#btnReset").click();	
				$("table tbody").html("");
				AddRows();
				document.getElementById("unitTestInfo").src="IATestData.asp";	
			}
			else{
	...
	function input2DCode(objValue){
	 ...
	 if (!$("#txt_boxsize").val()=="" &&  $("#txt_count").val()){
		...
		document.getElementById("unitTestInfo").src="IATestData.asp?txt_2d_code="+name;	
...
<input type="hidden" id="txt_pack_type" value="<%=packType%>" />
  ...
  <center>
  <iframe src="IATestData.asp" width="95%" height="320" scrolling="auto" frameborder="0" id="unitTestInfo"></iframe>
  </center>
