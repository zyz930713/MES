...
function resetSession()
{
	...
	location.replace("http://10.6.100.57:9810/packing/KEB_TSD_Maple.asp?JSQ=zero");
...
if JSQ="zero" then
 session(("CheckA"))=0
 session(("CheckB"))=0
 response.Redirect("http://10.6.100.57:9810/packing/KEB_TSD_Maple.asp")
end if 
...
<tr> 
  <td width="251" align="center"><span class="RED">Maple 2D Code ：</span></td>
...
'sql=" select summary_id,serialNumber,testTime,jobNumber,pcName,product,testResult,failName,TEST_ENVIRONMENT from [KEB_TEST_RESULT_DB].[dbo].[TEST_RESULT_SUMMARY]   a left join TEST_RESULT_MAIN_KEB b on(a.archive_id=b.ARCHIVE_ID) where serialnumber = '"&Dcode&"' order by  testTime desc"
'sql="select  * from package a ,serial_index b where a.serial_id=b.serial_id and serialnumber = '"&Dcode&"'" 
	strSql="select serial_id,testitem,testResult, jobnumber,pcname,testTime ,failname,rn from ( "
	strSql=strSql+"select b.serial_id,b.jobnumber, b.testitem, b.testTime,b.failname,b.pcname ,testResult,ROW_NUMBER() over(PARTITION by testitem order by b.testTime desc) rn   "
	strSql=strSql+"from serial_index a ,package b  where a.serial_id =b.serial_id  and   a.serialnumber = '"&Dcode&"'"
	strSql=strSql+") temp  where rn=1  order by testTime desc "	
	
	'response.Write(strSql)
rsTSD.open strSql,connTSD,1,1
...
if adfail="FAIL" then
 if  rsTSD("failName")="Micro-THDgap" then


'sqlV="select SERIAL_NUMBER,TEST_TIME,TEST_NAME,TEST_MARGIN from  [KEB_TEST_RESULT_DB].[dbo].[TEST_RESULT_DETAIL_S]   where  TEST_NAME   in ( 'Micro-THDgap')  and  SERIAL_NUMBER='"&Dcode&"'"
	
sqlV="select SERIAL_NUMBER,TEST_TIME,TEST_NAME,TEST_MARGIN from ("
sqlV=sqlV+" select *, ROW_NUMBER() over(PARTITION by [TEST_NAME] order by test_time desc) rn from  [TEST_RESULT_DETAIL_S]   where  TEST_NAME   in ( 'Micro-THDgap')  and  SERIAL_NUMBER='"&Dcode&"')"
sqlV=sqlV+"  temp where rn in ('1')"
	...
	do while not rsA.eof
		...
		sqlC="select * from CHECKABCSOL where paramSF='"&TEST_NAME&"' and Product_Name='Maple'"
