...
sqlV=sqlV+" select *, ROW_NUMBER() over(PARTITION by [SERIAL_NUMBER] order by test_time desc) rn from  [TEST_RESULT_DETAIL_S]   where  TEST_NAME   in ( 'FRES')  and  SERIAL_NUMBER='"&Dcode&"')"
