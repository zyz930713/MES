...
function checkTestResultKB(str2DCode,strPartNumber)
	...
	strsql="select * from Vw_Packing where serialnumber ='"&str2DCode&"'" 
	else
...
if PRODUCT="Explorer" then
		strSql="select  *  from Vw_resistance where  serialnumber = '"&str2DCode&"'" 
		rsTSD.open strSql,connTSD,1,3	
		K=0
		if rsTSD.eof then
		alarmMsg="没有测试RDC！！"	
		...
		while not rsTSD.eof 	
			 ...
			 RDCV=rsTSD("value")
			 if not  isNumeric(RDCV) then
			 alarmMsg="没有测试RDC数据！！"	
			 
			 else		 
			
					if csng(rsTSD("value"))< 0.4 and csng(rsTSD("value"))>0  then
						
					else			
						alarmMsg="RDC值"&rsTSD("value")&"测试不通过，不能打包!!"	
					end if
...
function checkTestResult(str2DCode,strPartNumber,test_type)
	...
	strSql=strSql+"select testitem, testTime, testResult,ROW_NUMBER() over(PARTITION by testitem order by testTime desc) rn  "
	strSql=strSql+"from Vw_Packing where serialnumber = '"&str2DCode&"'"
