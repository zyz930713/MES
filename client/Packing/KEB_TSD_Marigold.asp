...
if JSQ="zero" then
 session(("CheckA"))=0
 session(("CheckB"))=0
 response.Redirect("http://10.6.100.57:9810/packing/KEB_TSD_Marigold.asp")
end if 
...
sqlV="select SERIAL_NUMBER,TEST_TIME,TEST_NAME,TEST_MARGIN from  [TSD_PackageMarigold].[dbo].[TEST_RESULT_DETAIL_S]   where  TEST_NAME   in ( 'FRES')  and  SERIAL_NUMBER='"&Dcode&"'"
	...
	do while not rsA.eof
		...
		sqlC="select * from CHECKABCSOL where paramSF='"&TEST_NAME&"' and Product_Name='Marigold'"
