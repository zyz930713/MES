...
<script type="text/javascript">
	...
	function Save(){
		...
		$.post("PackingOperationNOTango1.asp",r,function(data,textStatus){
			//alert(data);
			var result=$.parseJSON(data);
			if(result.error==undefined){
				alert(result.message.replace("|","\n"));
				window.location="PackingOperationNOTango.asp"; 
				$("#btnReset").click();	
				$("table tbody").html("");
				AddRows();
	...
	function Print(){
		...
		$.ajax({
			type: "post",
			url: "PackingOperationNOTango1.asp",
			async:false,
			data:r,
...
if txt_boxid<>"" then
 			...
 			sql="select job_number, PART_NUMBER,job_number as subJob,PACKED_QTY,CUSTOMER, PACKED_LINE,CUSTOMER_LABEL_ID from job_pack_detail where  box_id='"&txt_boxid&"'"
		 ...
		 ''   if  not rs.eof then
			...
			exists2DInfo=exists2DInfo+"<tr><td colspan='2'><input type='text' name='txt_jobno' style='background-color:#666666'  readonly='true' value="&subJob&" /></td><td colspan='2'><input type='text' name='txt_packqty' style='background-color:#666666' readonly='true' value="&qty&" /></td></tr>"
    ...
    <thead>
	  ...
	  <tr >
	   ...
	   <tr >
        ...
        <td><input name="txt_custid" type="text" id="txt_custid" style="background-color:#666666"  value="<%=CUSTOMER_LABEL_ID%>" size="22" readonly="true"/></td>
	   </tr>
	   <tr >
