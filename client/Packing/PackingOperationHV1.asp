...
'			end if
	...
	IF isPartial="Y"  and  isTray="Y"  then			 
 					...
 					function PrintCustmerLable
			  			...
			  			sql="select to_char(sysdate,'IW') as week,to_char(sysdate,'Y')as year,to_char(sysdate,'DD') as day,to_char(sysdate,'MM') as mon ,to_Char(sysdate,'day','NLS_DATE_LANGUAGE = American') as dayw  from dual"
								rs.open sql,conn,1,3
									if not rs.eof then
										strYear=rs("year")
										strMon=rs("mon")
								...
								if custId="" then
									if customer ="MICROSOFT MOBILE" then	 
										countCondition=VENDOR_CODE&strYear&strWeek&getDays(strDays)&"B"&N10toC62(cint(right(packLine,1))+9,32)&getprodShift(prodShift)	
									else								 
								...
								rs.open sql,conn,1,3
									if rs.eof then
										...
										if customer ="MICROSOFT MOBILE" then
										custId=countCondition&repeatstring(rs("count_value"),"0",3)	
										else	
								...
								rs.close
									...
									if customer ="MICROSOFT MOBILE" then
										boxid=right(boxid,14)
										YMD=mid(boxid,5,5)  '从箱号得到年月日
										strYear=left(YMD,1)  '得到年
										strMon=mid(YMD,2,2)  '得到月
										strDay=mid(YMD,4,2)  '得到日
										YMD="201"+strYear+"-"+strMon+"-"+strDay 
										strWeek=DatePart("WW",dateadd("d",-1,YMD))
										strDays=Weekday(YMD,2)
										 
										countCondition=VENDOR_CODE&strYear&strWeek&strDays&"B"&N10toC62(cint(right(packLine,1))+9,32)&getprodShift(prodShift)	
										
										custId=countCondition&right(custId,3)	
								    end if
								end if
								...
								if member("error")="" then
									...
									set PrintCtl=server.createobject("PrintClass.PrintCtl") 										
										DateCode=201&strYear&strMon&strDay										
										 rtnCode=PrintCtl.PrintOEMKEBCustomerLabel(printName,customer,customerPN,COO,cstr(qty),custId,partNo)    																	
									'rtnCode="OK"
										if rtnCode<>"OK" then
				   ...
				   '调用打印功能
					...
					if printName="" then
						...
						boxIdN=boxId&"-"&i 
						    ...
						    if   customer="HuaWei" then 
								sql="select to_char(sysdate,'IW') as week,to_char(sysdate,'YY')as year,to_char(sysdate,'DD') as day,to_char(sysdate,'MM') as mon from dual"
								rs.open sql,conn,1,3
									if not rs.eof then
										strYear=rs("year")
										strMon=rs("mon")
							...
							elseif partNO="240326000031" then
							 ...
							 rtnCode=PrintCtl.PrintOEMKEBCustomerLabel(printName,customer,customerPN,COO,cstr(qty),custId,partNo)
							elseif partNO="240326300064" then
							rtnCode=PrintCtl.PrintOEMKEBCustomerLabel(printName,customer,customerPN,COO,cstr(qty),custId,partNo)
						...
						if LastQty>0 then
							...
							boxIdN=boxId&"-"&i
							 ...
							 rtnCode=PrintCtl.PrintOEMKEBCustomerLabel(printName,customer,customerPN,COO,cstr(LastQty),custId,partNo)
							else
		...
		case "3" 'check 2D code
			...
			if member("error")="" then	
			   ...
			   IF member("error")="" then	
				...
				sql="select part_number_tag,LINE_NAME, SHIFT,SUPPLIER ,nvl(JOB_GOOD_QTY,0) as JOB_GOOD_QTY from job where job_number='"&jobnumber&"' and  sheet_number='"&sheetnumber&"'"						
				rsAsyn.open sql,conn,1,3
				  ...
				  '  elseif   clng(rsAsyn("JOB_GOOD_QTY"))<=0  then 
						   	'member("error")="请确认是否已经关站，此工单可包装良品数量为零，不能打包"						
					else										
						...
						if member("error")="" and packType="FG" or PackType="SCRAP" then
							...
							if rsAsyn("part_number_tag")="990321100028" then
							partNo="240326000001"
							end if
							if packLine = "" then
							...
							if boxSize="" then
								...
								sql="select nvl(box_size,0) as box_size,CUSTOMER_DESC_EN from CUSTOMER_MATERIAL where item_name='"&partNo&"' and CUSTOMER_LABEL='"&customer&"'"
								'boxSize=0
								rs.open sql,conn,1,3
								if not rs.eof then
									boxSize=clng(rs("box_size"))
									CUSTOMER_DESC_EN=rs("CUSTOMER_DESC_EN")
								else
									member("error")="请确认工单料号与包装配置是否一致"		
								end if
								rs.close
							end if
						end if
						...
						'
								'if clng(rsAsyn("JOB_GOOD_QTY")) < boxSize then
...
'									PCount=JOB_GOOD_QTY
'								else
'									PCount=PCount+clng(JOB_GOOD_QTY)
'								end if
'								
'								if clng(PCount)>clng(boxSize) then
'									 JOB_GOOD_QTY=clng(JOB_GOOD_QTY)-(clng(PCount)-clng(boxSize))	
'									 PCount= clng(PCount)-(clng(rsAsyn("JOB_GOOD_QTY"))-clng(JOB_GOOD_QTY))					' 例： 一箱360 所有工单良品相加不能大于360
'								end if
							
								member("job_number")=inputJob
								member("part_number")=partNo
								member("pack_line")=rsAsyn("LINE_NAME")						
								member("boxsize")=boxSize						
						        member("packqty")=boxSize						     
								'member("JOB_GOOD_QTY")=boxSize '良品可包装数量
								'member("PCount")=boxSize
								member("prod_shift")=rsAsyn("SHIFT")
								member("supplier")=rsAsyn("SUPPLIER")
								member("remarks")=CUSTOMER_DESC_EN	
