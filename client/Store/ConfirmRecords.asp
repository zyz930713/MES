...
rs.close
SQL="select J.* from JOB_MASTER_STORE J where J.NID is not null "&where&order
'response.Write(SQL)
session("SQL")=SQL
rs.open SQL,conn,1,3
%>
...
<body>
<form name="form1" method="post" action="ConfirmRecords.asp">
...
<table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
<tr>
  <td height="20" colspan="10" class="t-c-green">KE入库清单</td>
</tr>
...
<tr>
  ...
  <td class="t-t-Borrow"><div align="center">确认人</div></td>
  <td class="t-t-Borrow"><div align="center">生产数量</div></td>
  <td class="t-t-Borrow"><div align="center">确认数量</div></td>
  <td class="t-t-Borrow"><div align="center"><img src="/Images/Accend.gif" alt="Click to accend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=J.STORE_TIME&ordertype=asc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'">确认时间<img src="/Images/Decend.gif" alt="Click to decend records" width="10" height="10" align="absmiddle" style="cursor:hand" onClick="javascript:location.href='<%=pagename%>?ordername=J.STORE_TIME&ordertype=desc&pagenum=<%=firstpage%>&pagesize_a=<%=pagesize_s%><%=pagepara%>'"></div></td>
...
%>
	<tr>
	  <td height="20"><div align="center"><%=i%></div></td>
		...
		<td><div align="center"><%if admin=false then%><%=rs("STORE_QUANTITY")%><%else%>
		  <input name="quantity<%=i%>" type="text" id="quantity<%=i%>" value="<%=rs("STORE_QUANTITY")%>">
		  <input type="save<%=i%>" name="save" id="save<%=i%>" value="保存" onClick="quantitysave('<%=i%>')">
		  <%end if%>
		</div></td>
...
%>
  <tr>
    <td height="20" colspan="10"><div align="center"><span id="inner_NoRecords">无记录</span></div></td>
  </tr>
<%end if
rs.close%>
</table>
