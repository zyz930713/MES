...
if jobnumber <> "" then
	SQL="SELECT * FROM STORE_PRINT A WHERE EXISTS (SELECT 1 FROM JOB_PACK_DETAIL WHERE INSTR(A.PRINT_MEMBERS,BOX_ID)>0 AND JOB_NUMBER LIKE '"&jobnumber&"%') "
...
<%if jobnumber <> "" then%>
	...
	<tr>
		...
		<td class="today"><div align="center">Box Id 箱号</div></td>
	  </tr>
	<%if not rs.eof then
		i=1
		while not rs.eof
	%>
		  <tr>
			...
			<td><div align="center"><a href="PrintPackList.asp?printid=<%=rs("NID")%>"><%=rs("NID")%></a></div></td>
