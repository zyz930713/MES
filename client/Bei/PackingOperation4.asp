...
'	strsqlPart="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and TEST_TYPE='RWK'"
'	'alarmMsg=strsqlPart
'	rsPart.open strsqlPart,conn,1,3
'	 if not rsPart.eof then
'		do while not rsPart.eof 	
'	    testNameA=testNameA&rsPart("test_name")&","
'		rsPart.movenext
'		loop
'
'        alarmMsg=testNameA
...
''
	...
	strSql="select linename,adfail,measuredatetime from ( "
	strSql=strSql+"select a.ad_id, linename, a.measuredatetime, adfail,ROW_NUMBER() over(partition by substring(a.linename,1,3) order by a.measuredatetime desc) rn "
	strSql=strSql+" from pvs.vw_adid_by_sn a,pvs.ad_serial b where a.ad_id=b.ad_id and b.serialnumber='"&str2DCode&"' "
	strSql=strSql+") temp  where  rn =1 and measuredatetime>'2013-06-01'"
	
	rsPVS.open strSql,connPVS,1,3	
	K=0
	do while not rsPVS.eof 	
	   
	   
		
		if  rsPVS("adfail")="False" then
		
		  testName=testName&left(rsPVS("linename"),4)&","
		
        else
			 if left(rsPVS("linename"),3)="AO2" then
			 alarmMsg="AO2没能测试通过"
			 end if
			 if left(rsPVS("linename"),3)="AO4" then
			 alarmMsg="AO4没能测试通过"
			 end if
			 
			 if packLine="9111" or packLine="9112" or packLine="9113"or packLine="9114" then
	           if left(rsPVS("linename"),3)="NO5" then
			   alarmMsg="NO5没能测试通过"
			   end if
	         end if 
	...
	rsPVS.close
	if testName<>"" then
		testName=left(testName,len(testName))
	end if
	...
	alarmMsg=""
		...
		testNameC="NO5"
		'alarmMsg=testName
		DDDD=cstr(instr(testName,testNameA))
		EEEE=cstr(instr(testName,testNameB))
		FFFF=cstr(instr(testName,testNameC))
	...
	'set rsPart=server.CreateObject("adodb.recordset")
'	strsqlPart="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"'"
'	'alarmMsg=strsqlPart
'	rsPart.open strsqlPart,conn,1,3
'	if not rsPart.eof then
'	
'	
'	
'	strSql="select test_name,alarm_msg from part_test_name where part_number = '"&strPartNumber&"' and test_name  in('"&replace(testName,",","','")&"') "
