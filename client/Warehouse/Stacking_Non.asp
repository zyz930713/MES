...
<script type="text/javascript">
	...
	function inputBoxId(objValue){
		if(event.keyCode==13){
			...
			var r=$("#form1").formSerialize()+"&asynid=7&plan_part="+planPart;				
...
<table id="table1" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    <thead>
      <tr>
        <td colspan="4" align="center" class="incent">Stacking 堆栈</td>
      </tr>
      <tr >
	  ...
	  <tr>
		<td height="29" align="right"> 计划<span class="red"> *</span></td>
        <td><%
		sql="select plan_id,CUSTOMER_NAME,part_number,to_char(TO_DATE(PLAN_DATE,'yyyy-mm-dd'),'mmdd') PLAN_DATE ,to_char(QUANTITY) QUANTITY from PACKING_PLAN_NON_TANGO where status in ('Initial','Wait') order by delivery_time"
			%>
                <select id="slt_plan" name="slt_plan"  size="2" >
			<option value=""></option>
            <%
				rs.open sql,conn,1,3
				while not rs.eof 
					response.Write("<option value='"+rs("plan_id")+"'>"+rs("part_number")+"-"+rs("PLAN_DATE")+"-"+rs("CUSTOMER_NAME")+"-"+rs("QUANTITY")+"</option>")
					rs.movenext
				wend
				rs.close
				%>
	  ...
	  <tr >
        ...
        <td><input name="txt_pallet" type="text" id="txt_pallet" onkeydown="if(event.keyCode==13){event.keyCode=9;}" size="17" /></td>
	  </tr>
	  <tr >
	...
	<tfoot>	
      <tr>
        <td colspan="4" align="center" ><br>		
			<input type="button" id="btn_print" value="打印" onClick="doPrint()"/>  			
			&nbsp;
			<input name="Close" type="button" id="Close" onClick="doCancel()" value="关闭">			
		</td>		
      </tr>
    </tfoot>
</table>
