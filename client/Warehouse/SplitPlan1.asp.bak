...
if len(asynid)>0 then	
	select case asynid
		...
		case "4" 'input box id to split pallet
			PLAN_ID=request("txt_PLAN_ID")
		
			'member("error")=palletId
			unitQty=0
			sql="select PLAN_ID,sum(packed_qty) as uniQty,count(IS_SHIPPED) as Is_Shipped from job_pack_detail where PLAN_ID='"&PLAN_ID&"' group by PLAN_ID"
			rs.open sql,conn,1,3
			if rs.eof then
				errMsg="This PLAN id("&PLAN_ID&") does not exist.|此计划号("&PLAN_ID&")不存在."
			elseif clng(rs("Is_Shipped")) >0  then
				errMsg="此计划"&PLAN_ID&"已扫描了出货不能解除。"
			else
...
sql=sql+"DELIVERY_ID,VENDOR_RECEIVE_DATE,CONFIRM_USER,CONFIRM_REMARKS,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME,GET_JSQ,BOXIDSTATUS,WHREC_JSQ,INSPECTIONPQC,BIND_BOX_ID FROM JOB_PACK_DETAIL "
			...
			'spilt box
			sql="UPDATE JOB_PACK_DETAIL SET PALLET_ID='',PLAN_ID='',STACK_USER='',STACK_TIME='' "
			sql=sql+strWhere
			conn.execute(sql)
...
'			
		...
		case "6" 'split pallet
			...
			PLAN_ID=trim(request("txt_PLAN_ID"))
...
sql=sql+"DELIVERY_ID,VENDOR_RECEIVE_DATE,CONFIRM_USER,CONFIRM_REMARKS,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME,GET_JSQ,BOXIDSTATUS,WHREC_JSQ,INSPECTIONPQC,BIND_BOX_ID FROM JOB_PACK_DETAIL "
			...
			sql="UPDATE JOB_PACK_DETAIL SET PALLET_ID='',plan_id='',STACK_USER='',STACK_TIME='',WHREC_USER='"&opCode&"',WHREC_TIME='',GET_USER='"&getCode&"',GET_TIME=SYSDATE,get_JSQ=decode(get_jsq,null,1,get_jSQ+1),BOXIDSTATUS='' "
			sql=sql+strWhere
			conn.execute(sql)
