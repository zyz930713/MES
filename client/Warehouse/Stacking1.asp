...
if len(asynid)>0 then	
	...
	case "7" 'input box id
			...
			if left(planPart,12)="990321100028" then
			planPart="240326000001"
			end if
			...
			unitQty=0
			boxQty=1
			sql="select part_number,pallet_id,sum(packed_qty) as packed_qty,sum( case when WHREC_Time is null then 0 else 1 end ) as WT,BOXIDSTATUS  from job_pack_detail where box_id='"&boxId&"'" 			 	
			sql=sql+" group by part_number,pallet_id,BOXIDSTATUS"
			...
			elseif rs("BOXIDSTATUS")="MRB" then
			 errMsg="该箱号("&boxId&")入的是MRB，不能直接发货"
			else
			...
			if errMsg="" then
				sql="select quantity,status,pack_qty from PACKING_PLAN_NON_TANGO where plan_id='"&planId&"'"
				rs.open sql,conn,1,3
				if not rs.eof then
		...
		case "6" 'Save data and print pallet label
			...
			sql="SELECT CUSTOMER_NAME,QUANTITY FROM PACKING_PLAN_NON_TANGO where plan_Id='"&planId&"'" 					
			rs.open sql,conn,1,3
			if not rs.bof then
			...
			elseif boxIdList="" then 'reprint pallet label
				sqll="select sum(Packed_Qty)  Packed_Qty from Job_Pack_Detail where pallet_id='"&palletId&"'"
				rs.open sqll,conn,1,3
				if isnull(rs("Packed_Qty")) then
					errMsg="拍号不存在，请确认拍号。"
				else
					txt_unitqty=rs("Packed_Qty")
				end if
				rs.close	
			...
			if boxIdList<>"" then
				...
				sql="update PACKING_PLAN_NON_TANGO set status='Wait',pack_qty=pack_qty+"&newUnitQty&" where plan_id='"&planId&"'"	
				conn.execute(sql)	
			end if
		...
		case "2" 'Save data and print pallet label
			...
			txt_unitqty=request("txt_unitqty")
			conn.beginTrans
			sql="SELECT CUSTOMER_NAME,QUANTITY FROM PACKING_PLAN where plan_Id='"&planId&"'" 					
			rs.open sql,conn,1,3
			if not rs.bof then
			CUSTOMER_NAME=rs("CUSTOMER_NAME")
			QUANTITY=rs("QUANTITY")
...
sql=sql+"DELIVERY_ID,VENDOR_RECEIVE_DATE,CONFIRM_USER,CONFIRM_REMARKS,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME,GET_JSQ,BOXIDSTATUS,WHREC_JSQ,INSPECTIONPQC,BIND_BOX_ID FROM JOB_PACK_DETAIL "
			...
			if project="Tango" then			
			sql="select * from PACKING_PLAN where plan_id='"&plan_id&"'"
			else
			sql="select * from PACKING_PLAN_NON_TANGO where plan_id='"&plan_id&"'"
			end if 
			rs.open sql,conn,1,3
			if not  rs.eof then
