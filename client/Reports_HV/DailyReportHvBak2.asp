...
%>
	<table width="1020" border="0" cellspacing="0" cellpadding="0" bgcolor="#7BB1DB">
	  ...
	  <tr>
		<td style="color:#fff;text-align:center;"><a href="DailyReportHv.asp?Getdata=ok&ProductName=<%=ProductName%>&Adate=<%=dateadd("d",-1,Adate)%>"><<</a>　<%=Adate%>　<a href="DailyReportHv.asp?Getdata=ok&ProductName=<%=ProductName%>&Adate=<%=dateadd("d",1,Adate)%>">>></a></td>
	...
	while not RsL.eof
		...
		while not RsD.eof
			...
			RemarkStrSum = RemarkStrSum & "<table width='1020' border='1' cellpadding='0' cellspacing='0'>" & TableStr & "</table>" & WriteRemarkStr(ProductName,LineName,DName,Adate)
			RsD.movenext
		Wend
		RsD.Close
	...
	OraLoopSql = "SELECT PRODUCTNAME,'All' LINENAME,'All Day' DNAME,'AllShift' ShiftName,count(productname) CountDay,sum(NVL(OPENTIME,0)) OPENTIME,sum(NVL(STOPTIME,0)) STOPTIME,sum(NVL(T1G,0)) T1G,sum(NVL(T1B,0)) T1B,sum(NVL(C1G,0)) C1G,sum(NVL(C1B,0)) C1B,sum(NVL(C2G,0)) C2G,sum(NVL(C2B,0)) C2B,sum(NVL(C3G,0)) C3G,sum(NVL(C3B,0)) C3B,sum(NVL(C4G,0)) C4G,sum(NVL(C4B,0)) C4B,sum(NVL(C5G,0)) C5G,sum(NVL(C5B,0)) C5B"
	OraLoopSql = OraLoopSql & ",SUM(NVL((select sum(c3g) from report_hv_data where '"&WeekFirstDay&"' < = adate and adate < = '"&Adate&"' AND ProductName = '"&ProductName&"' AND LINENAME = '"&LineName&"' AND dname = rhd.dname),0)) C3GW"
	OraLoopSql = OraLoopSql & ",SUM(NVL((select sum(c5g) from report_hv_data where '"&WeekFirstDay&"' < = adate and adate < = '"&Adate&"' AND ProductName = '"&ProductName&"' AND LINENAME = '"&LineName&"' AND dname = rhd.dname),0)) C5GW"
	OraLoopSql = OraLoopSql & " FROM report_hv_data RHD"
	...
	OraLoopSql = "SELECT PRODUCTNAME,'All' LINENAME,'All Night' DNAME,'AllShift' ShiftName,count(productname) CountDay,sum(NVL(OPENTIME,0)) OPENTIME,sum(NVL(STOPTIME,0)) STOPTIME,sum(NVL(T1G,0)) T1G,sum(NVL(T1B,0)) T1B,sum(NVL(C1G,0)) C1G,sum(NVL(C1B,0)) C1B,sum(NVL(C2G,0)) C2G,sum(NVL(C2B,0)) C2B,sum(NVL(C3G,0)) C3G,sum(NVL(C3B,0)) C3B,sum(NVL(C4G,0)) C4G,sum(NVL(C4B,0)) C4B,sum(NVL(C5G,0)) C5G,sum(NVL(C5B,0)) C5B"
	OraLoopSql = OraLoopSql & ",SUM(NVL((select sum(c3g) from report_hv_data where '"&WeekFirstDay&"' < = adate and adate < = '"&Adate&"' AND ProductName = '"&ProductName&"' AND LINENAME = '"&LineName&"' AND dname = rhd.dname),0)) C3GW"
	OraLoopSql = OraLoopSql & ",SUM(NVL((select sum(c5g) from report_hv_data where '"&WeekFirstDay&"' < = adate and adate < = '"&Adate&"' AND ProductName = '"&ProductName&"' AND LINENAME = '"&LineName&"' AND dname = rhd.dname),0)) C5GW"
	OraLoopSql = OraLoopSql & " FROM report_hv_data RHD"
	...
	response.write "<tr><td><table width='1020' border='1' cellpadding='0' cellspacing='0'><tr style='background:#FFFFFF;'><td align='left'>" & RemarkStrSum & "</td></tr></table><br>"
end if
...
function WriteTableStr(SqlStr)
	...
	if not LoopRs.eof then
		...
		elseif DNAME = "All Day" then
			Shift = "All Day"
			TrStyle = "style='background:#FFD3D3;text-align:center;'"
		elseif DNAME = "All Night" then
			Shift = "All Night"
			TrStyle = "style='background:#EBD3D3;text-align:center;'"
...
'提取Remark开始
function WriteRemarkStr(ProductName,LineName,DName,ADATE)
	RemarkStr = ""
	RemarkStrSum = ""
	RemarkStrSum = "<tr><td><table width='1020' border='1' cellpadding='0' cellspacing='0'>" & ListStr & "</table></td></tr><tr style='background:#FFFFFF;'><td align='left'>"
	...
	rs.close
	set rs = nothing
	WriteRemarkStr = RemarkStrSum & RemarkStr
end function
'提取Remark结束
%>
