...
<%session("Page_Role") = ",Split_Temp_Box"%>
...
<script type="text/javascript">
	...
	function inputBoxId(objValue){
		if(event.keyCode==13 && objValue!=""){
			...
			var data = window.showModalDialog("Split_Temp_BOX1.asp?"+r,window,"dialogHeight:100px;dialogWidth:100px;alwaysLowered:yes");
			...
			if(result.error==undefined){						
						...
						$("#hidBoxIdQTY").val($("#hidBoxIdQTY").val()+","+objValue+"-"+result.qty);	
	...
	function doSplit(){		
		...
		var data = window.showModalDialog("Split_Temp_BOX1.asp?"+r,window,"dialogHeight:10px;dialogWidth:10px;alwaysLowered:yes");
		var result=$.parseJSON(data);
		var result=$.parseJSON(data);
				if(result.error==undefined){
...
<table id="table1" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    <thead>
      <tr>
        <td colspan="4" class="t-t-DarkBlue" align="center">解箱号处理</td>
      </tr>
      <tr >
        <td align="right">工号<span class="red"> *</span></td>
        <td><input type="text" id="txt_opcode" name="txt_opcode"  value="<%=session("UserCode")%>" readonly="readonly"  style="background-color:#666666"/></td>
	  </tr>	  
	...
	<tfoot>	
      <tr>
        <td colspan="4" align="center" ><br>		
			<input name="btnSplit" type="button" id="btnSplit" onClick="doSplit()" value="解箱">			
			&nbsp;
