...
<script type="text/javascript">
	...
	function inputBoxId(objValue){
		if(event.keyCode==13){
			...
			var data = window.showModalDialog("Split_Code1.asp?"+r,window,"dialogHeight:10px;dialogWidth:10px;alwaysLowered:yes");
	...
	function doPrint(){		
		...
		if($("#txt_inputboxid").val()==""){
			...
			document.getElementById("txt_inputboxid").focus();
			return false;
...
<body bgcolor="#339966" onload= "javascript:document.all.txt_inputcode.focus();">
...
<table id="table1" border="1" align="center" cellpadding="0" cellspacing="0" bordercolorlight="#000099" bordercolordark="#FFFFFF">
    <thead>
      ...
      <tr align="left" >
        ...
        <td><input name="txt_opcode" type="text" id="txt_opcode" onkeydown="if(event.keyCode==13){event.keyCode=9;}" size="23"   value="<%=session("UserCode")%>" readonly="readonly"  style="background-color:#666666"/></td>
	  </tr>
      <tr align="left" >
        <td width="68">箱号<span class="red"> *</span></td>
        <td width="211"><input name="txt_inputboxid" type="text" id="txt_inputboxid"  value="<%=request("txt_inputboxid")%>"size="23" onKeyDown="if(event.keyCode==13){event.keyCode=9;}" /></td>
	  </tr>
	  ...
	  <tr >
        ...
        <td><input name="txt_inputcode" type="text" id="txt_inputcode" onKeyDown="inputBoxId(this.value)" size="23"/></td>
	  </tr>
	  <tr >
