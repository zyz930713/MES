...
if len(asynid)>0 then	
	select case asynid
		...
		case "1" 'input box id
			...
			if not rs.eof then
			    qty=rs("PACKED_QTY")
			   errMsg="该箱号("&boxId&")存在临时库中("&qty&")，请确认箱号是否重复.请找出另一个箱子"
			end if
			rs.close
			'use exist pallet
	...
	sql="insert into JOB_PACK (BOX_ID,PART_NUMBER,PACKED_QTY,PACKED_LINE,PROD_SHIFT,BOXIDSTATUS,WHREC_USER,WHREC_TIME)"
	sql=sql+"  VALUES  ('"+boxId+"','"+part_number+"',"&unitQty&",'"&PACKED_LINE&"','"&PROD_SHIFT&"','"+WHlocation+"','"+opcode+"',sysdate)"
