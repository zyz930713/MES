...
var strTitle="NCMR Ticket ( Follow Up ) |异常处理单记录单 （措施验证）"
...
var strCorrect7="Skip Long-term Action|纠正措施被跳过"
var arrCorrect7=strCorrect7.split("|")
...
var strNote="Comments|备注"
var arrNote=strNote.split("|")
var strWaiting="Action Status|跟踪状态"
var arrWaiting=strWaiting.split("|")
...
try{inner_Person7.innerText=arrPerson7[<%=session("language")%>]}catch(e){} 
...
try{inner_DueDate2.innerText=arrDueDate1[<%=session("language")%>]}catch(e){}
...
try{inner_Correct7.innerText=arrCorrect7[<%=session("language")%>]}catch(e){}  
...
try{inner_Waiting.innerText=arrWaiting[<%=session("language")%>]}catch(e){}
try{inner_Confirm1.innerText=arrConfirm1[<%=session("language")%>]}catch(e){} 
