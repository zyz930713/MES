...
function selectCheck(testnum)
{
   ...
   */
	...
	function ajax_post()
	{
  $.post("Barcode_END_Save.asp",{Dcode:QueryStringTest,SNNO:SNNO},
  function(data){
...
return str.replace(/(\s*$)/g,"");  
	...
	function NPISaveData()
	{
	
	     if(confirm("确定要接受吗？"))
   { 
			document.form1.action="EndProd_Save.asp?PTCstate=End";
...
<td width="45%" height="139" align="left" bgcolor="#339966" class="STYLE5"><div id="JSQ"><%=rsJS("ReturnNO")%></div></td>
...
<table width="540" height="7" border="0" align="center" cellpadding="0" cellspacing="0">
<tr align="center"> 
<td width="" height=7><input name="btnclose" type="button"  id="btnclose"  value="Close 关闭" onClick="javascript:window.close();">
<input name="btnclose" type="button"  id="btnclose"  value=" 继续扫描 " onClick="javascript:window.location.reload();">
<input name="btnclose2" type="button"  id="btnclose2"  value=" 结束页面 " onClick="javascript:window.open('ENDprod.asp');">
<input type="hidden" id="SNNO" name="SNNO" value="<%=request("SNNO")%>"><%
	 set rsEnd=server.CreateObject("adodb.recordset")
			 sql="select * from PTC_SN where SNNO='"&SNNO&"'"
	
	 rsEnd.open sql,conn,1,1
	 if rsEnd.eof And rsEnd.bof then
    
   	 else
	 
	if rsEnd("LendNO")=rsEnd("ReturnNO") then %>

<input name="btnclose2" type="button"  id="btnclose2"  value="NPI 接收" onClick="NPISaveData();">
<%else%>
