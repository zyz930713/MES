...
<script type="text/javascript">
	...
	function inputJobNumber(objValue){
		if(event.keyCode==13){
			...
			else if(jobNo.indexOf("RWK")!=0 && jobNo.indexOf("TKB")!=0){
				alert("This job number is not rework job number.\n此工单不是重工工单。");
				document.getElementById("txt_job").select();
				return false;
			}
	      ...
	      document.getElementById("txt_job").disabled=true;
						
		    var txt_job=$("#txt_job").val();
		    var r=$("#form1").formSerialize()+"&asynid=3&txt_job="+txt_job;				
			
			$.ajax({
				type: "post",
				url: "ReworkReceive1.asp",
				async:false,
				data:r,
				success:function(data,textStatus){
					var result=$.parseJSON(data);
					if(result.error==undefined){
						$("#JobNumberstate").val(result.JobNumberstate);
					document.getElementById("txt_job").disabled=true;
	...
	function inputBoxId(objValue){
		...
		//	if(objValue.indexOf("FG")!=0){
		    ...
		    var txt_job=$("#txt_job").val();
		    var r=$("#form1").formSerialize()+"&asynid=1&txt_job="+txt_job;	
	...
	function doPrint(){		
		...
		if(totalQty>0){
			...
			if(jobNo==""){
			 ...
			 var txt_job=$("#txt_job").val();
		    var r=$("#form1").formSerialize()+"&asynid=2&txt_job="+txt_job;	
		...
		document.getElementById("txt_job").disabled=false;
		//if($("#txt_subjob").val() != "" ){
    ...
    <thead>
	  ...
	  <tr>
        ...
        <td><select id="sltStartStation" name="sltStartStation">
				<option value=""></option>
				<%sql="select nid,station_name,station_chinese_name from station_new where nid in ('SA00000989','SA00000990') order by nid"
				...
				rs.close
				%>
				<option value="Packing">Packing(包装)</option>
			</select>
		</td>
      </tr>
	  <tr >
        <td align="right">Job Number 工单号<span class="red"> *</span></td>
        <td><input type="text" id="txt_job" name="txt_job" onKeyDown="inputJobNumber(this.value)" />
        <input  type="button" name="JobNumberstate" id="JobNumberstate" value="NO" /></td>
	  </tr>
	  <tr >
	...
	<tfoot>	
      <tr>
        <td colspan="4" align="center" ><br>		
			<input type="button" id="btn_print" value="生产子工单" onClick="doPrint()"/>  
			&nbsp;
