...
if len(asynid)>0 then
	...
	select case asynid
		...
		case "1" 'input box id	
			...
			if not rs.eof then
			   errMsg="该箱号("&boxId&")存在临时库中，不能返检"
			end if
			rs.close

			 sql="select part_number,count(pallet_id)  as pallet_id,sum(packed_qty) as qty,Packed_Line,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME from job_pack_detail where box_id='"&boxId&"'" 			 	
			sql=sql+" group by part_number,Packed_Line,pallet_id,WHREC_USER,WHREC_TIME,GET_USER,GET_TIME"
			
			
			rs.open sql,conn,1,3
			if rs.eof or isnull(rs("qty")) then
				errMsg="This box id("&boxId&") does not exist.|此箱号("&boxId&")不存在."
			
			elseif  isnull(rs("WHREC_TIME"))=false then
				errMsg="该箱号("&boxId&")没有出库，所以不能返工!"
			elseif clng(rs("pallet_id"))>0 then
				errMsg="该箱号("&boxId&")已堆拍，请先解拍号再返工!"  			
			else
				...
				if rs.eof then			
					...
					if rsTicket.eof then
						...
						if rsL.eof then
							...
							sql="select nid,factory_id,part_rule from part where LINES_INDEX like '%"&line_id&"%' order by meet_priority, part_rule desc"	
